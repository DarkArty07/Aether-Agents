...
**Status:** this source revision defines `1.0.0rc7` / `1.0.0-rc.7` as a **local-only compatibility bridge** for the #497 updater repair. It intentionally retains RC6-identical Morfeo SOUL and canonical contract skills; it does **not** deliver the #495 planning guidance. Its local annotated tag identity is `v1.0.0-rc.7`; neither this source nor a local tag proves what is installed. Query `aether doctor` for the active version and require a reviewed commit, isolated qualification and an explicit managed cutover before claiming activation. `release_impact = patch`, `release_action = prepare`, `release_channel = prerelease`; no tag is pushed and no GitHub/package publication is authorized. RC1 remains [published but rejected](https://github.com/DarkArty07/Aether-Agents/releases/tag/v1.0.0-rc.1) and must not be activated; earlier local tags remain immutable, and #261 remains open. This is **not** stable `1.0.0`, a PyPI release or WSL2 qualification.
