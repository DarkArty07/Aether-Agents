...
**Status:** the checked-in tree carries the owner-authorized `1.0.0rc4` release candidate, the successor to the local rc.3 candidate: package version `1.0.0rc4` and annotated tag `v1.0.0-rc.4`. The tag is a local-only candidate that is not pushed and not published in this objective, with `release_impact = patch`, `release_action = prepare`, `release_channel = prerelease`. The local rc.2 and rc.3 tags and activation history remain immutable and are not acceptance; [`v1.0.0-rc.1`](https://github.com/DarkArty07/Aether-Agents/releases/tag/v1.0.0-rc.1) remains published and byte-immutable but rejected, and must not be activated. This milestone is bounded and pre-stable: explicitly **not** stable `1.0.0`, **not** a PyPI or other package-index publication, and **not** a WSL2 qualification result. Issue #261 therefore stays open with the stable, PyPI/OIDC and WSL2 gates outstanding. Feature expansion and nonessential Hermes changes remain frozen while the rolling reliability gate is qualified.
