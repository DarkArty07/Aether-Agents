...
`aether init` initializes an existing Git repository root as an Aether Project: it writes the portable `.aether/project.toml` marker validated against `project.schema.json`, and maps that UUID to exactly one native Hermes Project matched by **exact primary path**. It also guarantees the canonical ignore policy — marker and finalized Objective Contracts trackable, local drafts and other `.aether/` state ignored — by appending to `.gitignore` only when needed, never editing existing rules, and refusing if appending cannot achieve it. It never creates, modifies, or archives a Hermes Project, never matches by name or approximate path, and never overwrites an existing marker; ambiguous, mismatched, or conflicting identity is refused. Initializing Git in an empty greenfield directory is not performed by this build: `init` requires an existing repository root.
