...
def test_exact_public_lifecycle_uses_real_plugin_profiles_query_and_recovery(
    ...
    """Exercise the exact fork and release lifecycle with a confined gateway unit.
    ...
    accepted as the RC runtime. The gateway unit is generated by that fork but
    installed only by the test runner under ``tmp_path``: the real Hermes install
    refuses temporary homes to protect the operator's user service.
    """
    ...
    isolated_systemd = isolated_home / ".config" / "systemd" / "user"
    ...
    def confined_hermes_runner(
        cmd: list[str], env: dict[str, str], cwd: Path
    ) -> subprocess.CompletedProcess[str]:
        """Generate real target-Hermes bytes without a temp-home service write."""
        # Virtualenv bin/python normally symlinks to a shared interpreter;
        # the invocation path, not its symlink target, must stay in the fixture.
        assert Path(os.path.abspath(cmd[0])).is_relative_to(tmp_path.resolve())
        assert Path(env["HOME"]).resolve() == isolated_home.resolve()
        assert Path(env["HERMES_HOME"]).resolve() == store.profile_home("morfeo").resolve()
        assert cwd.resolve() == store.profile_home("morfeo").resolve()
        assert "gateway" in cmd
        unit = isolated_systemd / lifecycle.AETHER_GATEWAY_UNIT
        if cmd[-1] == "uninstall":
        ...
        generated = subprocess.run(
            [
                ...
                "print(generate_systemd_unit(), end='')",
            ],
            check=True,
            capture_output=True,
            text=True,
        ...
        assert f"HERMES_HOME={env['HERMES_HOME']}" in generated.stdout
        unit.write_text(generated.stdout, encoding="utf-8")
        return subprocess.CompletedProcess(cmd, 0, "", "")

    manager = LifecycleManager(
        store=store,
        python_executable=Path(sys.executable),
        hermes_runner=confined_hermes_runner,
    )
