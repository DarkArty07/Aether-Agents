...
| Etalides ACP stall (not model-specific) | `talk_to(etalides)` polls with no messages or tool calls (thoughts show "formulating" indefinitely) | Close session, use `delegate_task(toolsets=["web"])` as fallback. Root cause: ACP session-level issue, confirmed with minimax-m2.7. Full Hermes restart required when changing Daimon configs |
| model_normalize.py misroutes deepseek models | Setting `model: deepseek-v4-flash` + `provider: opencode-go` in Daimon config → HTTP 402 "unable to verify membership" because model_normalize intercepts `deepseek-*` prefixes and routes to kimi-coding endpoint (`api.kimi.com/coding`), ignoring the `provider: opencode-go` field | **Workaround:** Use `qwen3.6-plus` with `provider: opencode-go` — it's in the opencode-go catalog and not intercepted. **Proper fix:** Patch `model_normalize.py` (~line 396) to add `if provider == "opencode-go"` case that preserves model name as-is, like the existing `opencode-zen` handler. Affects ANY Daimon config using a `deepseek-*` model name with opencode-go |
