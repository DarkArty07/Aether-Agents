...
| 2 — INVESTIGAR | `talk_to(etalides)` or `run_workflow(research)` | `delegate_task` for quick lookups |
| 3 — DISEÑAR | Direct conversation with user | `talk_to(hefesto)` for technical feasibility |
| 4 — PLANIFICAR | `talk_to(ariadna)` for review | Direct planning with user |
| 5 — PROGRAMAR | `run_workflow(feature|bug-fix|refactor)` | `delegate_task` for isolated changes |
