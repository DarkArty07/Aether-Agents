...
| `talk_to()` timeout after exactly 300s | Olympus server.py clamp was limiting wait to max 300s | Fixed 2026-04-29: clamp raised to 600s, default to 300s |
| `talk_to()` stalls on large prompt (kawaii thoughts only, no messages) | ACP stall — Daimon never completes response for long/complex prompts | Workaround: use `delegate_task` for tasks with long prompts |
...
- **Olympus `talk_to` wait timeout was clamped to 300s (FIXED 2026-04-29)**: `_handle_talk_to` in `server.py` had `timeout = args.get("timeout", 120)` with `clamp = min(max(timeout, 1), 300)`, meaning Daimon tasks longer than 5 minutes would always time out. Combined with the hermes-agent MCP config `timeout: 600`, the server-level clamp was the bottleneck. **Fix**: Default raised to `300s` (5 min), max clamp raised to `600s` (10 min). For tasks exceeding 10 minutes, use `delegate_task` (bypasses Olympus entirely) or break into smaller sub-tasks. **Diagnostic**: If `talk_to` returns status "timeout" after exactly 300s, the old clamp was the cause. Check with `grep 'min(max(timeout' src/olympus/server.py` — should show `600`, not `300`.
- **ACP stall on large prompts (WORKAROUND)**: Sending a very long/complex prompt to a Daimon via `talk_to` can cause the ACP session to stall — the Daimon produces only kawaii thoughts ("reflecting...", "pondering...") but never produces actual messages. The session stays `active` with `messages: []` indefinitely. **This is NOT the same as the timeout issue** — the Daimon is alive but its response never completes. Short prompts (1-2 sentences) work reliably. **Workaround**: For tasks requiring long prompts (implementation plans, multi-file changes), use `delegate_task` with appropriate toolsets instead of `talk_to`. Sub-agents bypass Olympus ACP and don't suffer from this stall.
- **read_file → write_file corruption (CRITICAL PITFALL, commit 18fd0a1)**: When a subagent (via `delegate_task`) reads a file with `read_file` and then rewrites it with `write_file`, the line number prefixes from `read_file` output (`     1|`, `     2|`, etc.) get embedded as actual file content. Example: `# Hermes — Orchestrator` becomes `     1|# Hermes — Orchestrator`. **This is especially dangerous for SOUL.md files** (always loaded as Slot #1 system prompt) — corrupted SOUL.md breaks Daimon identity silently. **Root cause**: The LLM doesn't distinguish display metadata from file content. **Prevention**: (1) Use `patch` for targeted edits instead of read+rewrite. (2) If full rewrite is needed, instruct the subagent explicitly: "Strip all line number prefixes before writing." (3) Verify with `head -3 file` after any subagent file modification. **Recovery**: `sed -i 's/^[[:space:]]*[0-9]\+|//' file` strips all line-number prefixes. Verify tables/markdown not damaged — legitimate `|` lines (tables) start with `| ` not `digits|`.
- **Files in .gitignore still tracked by git**: Adding a file/path to `.gitignore` does NOT remove it from git tracking if it was already committed. Git only ignores untracked files. **Symptom**: `.eter/`, `.hermes/`, runtime files appear on GitHub despite being in `.gitignore`. **Fix**: `git rm --cached -r <path>` removes from the index without deleting from disk. The `.gitignore` then prevents re-tracking. **Prevention**: Add to `.gitignore` BEFORE first commit. After running `git rm --cached`, commit the removal explicitly so the deletion reaches the remote.