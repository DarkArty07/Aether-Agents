...
## SOUL.md + Skills Adaptation for Workflows (Phase 3 — COMPLETED in commit f14c3d8)

When the workflow engine was built (Phases 1-2.5), SOUL.md files and workflow Skills were designed for the **fire-and-forget model** (`talk_to` — one prompt, one response). Phase 3 adapted all files for the new workflow system.

### Gap Summary — All Resolved

| Gap | Impact | Affected Files | Status |
|-----|--------|----------------|--------|
| No `run_workflow` routing in Hermes SOUL.md or orchestration skill | Hermes doesn't know when/how to use workflows vs `talk_to` | Hermes SOUL.md, orchestration SKILL.md | ✅ Fixed |
| No HITL handling instructions | Hermes won't know what to do when a workflow interrupts | Hermes SOUL.md, orchestration SKILL.md | ✅ Fixed |
| Daimon SOULs only describe fire-and-forget model | Daimons won't leverage accumulated context from prior nodes | All 5 Daimon SOUL.md files | ✅ Fixed — all have "In Workflow Context" |
| Daimon workflow Skills lack "In Workflow Context" guidance | Skills don't adapt output format for workflow context vs manual `talk_to` | All 5 Daimon workflow Skills | ✅ Fixed — all have "Workflow Context Note" |
...
### Changes Applied (commit f14c3d8)

| Status | File | Change Description |
|--------|------|--------------------|
| ✅ | `orchestration/SKILL.md` | Replaced old 3-workflow section with 6 canonical workflows + routing matrix + HITL few-shots + `run_workflow` examples + updated pre-flight checklist |
| ✅ | `Hermes SOUL.md` | Added "Workflow Orchestration" section: routing decision, HITL handling, parameter guide by workflow type |
| ✅ | `workflow-design/SKILL.md` | Added "Hermes Usage Guide" section with few-shots per workflow type, parameter recommendations, interrupt handling patterns |
| ✅ | Each Daimon SOUL.md (5 files) | Added "In Workflow Context" section: how to interpret accumulated context, adapt output, handle `workflow_type` parameter |
| ✅ | Each Daimon Skill (5 files) | Added "Workflow Context Note": adapt output format when receiving enriched context from prior nodes |
| ✅ | `ariadna/SOUL.md` | Note about `project-init` participation |

### Implementation Approach (EXECUTED — commit f14c3d8)

The `refactor` workflow was used to implement these changes:
1. Etalides mapped impact (13 files, 26 stale references, no breaking changes) → ✅
2. `scope_review` HITL — Christopher approved the scope → ✅
3. Hefesto implemented the changes (14 files, +804/-54 lines) → ✅
4. Athena verified cross-file consistency (all references match, terminology aligned) → ✅

Result: All 6 gaps resolved. No remaining stale references to old workflows.