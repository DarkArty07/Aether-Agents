...
> **CORRECTION (2026-06-28):** This document uses `file-read` as a toolset name throughout. **`file-read` is NOT a valid toolset name** — it is silently ignored in both `toolsets` and `disabled_toolsets` config. The correct name is **`file`** (bundles `read_file`, `write_file`, `patch`, `search_files`). The actual implementation used `file` correctly (see verification line below: "verified: web, browser, file, terminal"). Replace all `file-read` references with `file` when applying this plan.
