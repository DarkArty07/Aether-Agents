...
## MCP Server Crash from Oversized Delegation Prompts

**Observed 2026-06-22:** An extremely large `talk_to(action="delegate")` prompt (multi-kilobyte, containing full implementation specs for multiple files, file contents, and detailed constraints) caused the olympus_v3 MCP server to become unreachable. The symptom: `talk_to` and `discover` both return "MCP server 'olympus_v3' is unreachable."

**Root cause:** The MCP server process crashes or hangs processing the oversized JSON-RPC message. The exact failure point (JSON parsing, memory, timeout) wasn't isolated, but the mitigation is known.

**Mitigation:** Split large batch tasks into multiple smaller `delegate` calls — one file at a time. Instead of one prompt with 3 file specs, send 3 prompts each with 1 file spec. This also improves Daimon focus and result quality.

**Recovery:** If the server crashes, check for zombie processes (Pitfall #24 in olympus-acp-debugging.md), kill orphans, and restart the gateway.
