...
4. **Update wrapper script** (`~/.local/bin/hermes`)
   ```bash
   #!/bin/bash
...
## Profile Architecture (v0.8.3+)

As of v0.8.3, the orchestrator runs as the **default profile** (no `-p` flag), not as a named profile. Named profiles are only for Daimons.

| Instance | Path | Purpose | Model | Interface |
|----------|------|---------|-------|-----------|
...
- The same hermes binary (`~/.local/bin/hermes` → `.venv-hermes/bin/hermes`)
- The same API keys (but different `.env` files)
- The same skills directory structure (but separate copies)

**After migration**: Update:
1. `~/.local/bin/hermes` wrapper — remove `-p orchestrator` (just `exec hermes "$@"`)
...
**Profiles with templates**: `ariadna`, `hefesto`, `etalides`, `athena`, `daedalus`, `ictinus`

The orchestrator runs as the default profile from `home/` (no separate profile directory). The `profiles/hermes/` and `profiles/orchestrator/` directories were removed in v0.8.3 — `hermes` is a reserved profile name that causes gateway crashes, and `orchestrator` is no longer needed since the orchestrator uses the default profile.
...
**Pitfall**: Never re-track `config.yaml` files in git. They contain machine-specific paths. Only `config.yaml.template` files are tracked. The `.gitignore` section `# --- Per-profile live config (generated from templates) ---` lists all Daimon profile configs. The default profile's `home/config.yaml` is also gitignored (at the `home/` level).

**Note**: As of v0.8.3, the orchestrator runs as the default profile from `home/`. The `profiles/hermes/` directory (reserved name, stale config with olympus v2) and `profiles/orchestrator/` directory (migrated to default) have been deleted.
...
| `home/profiles/hermes/` | Reserved profile name (gateway crash), SOUL.md was generic, agent-hooks duplicated in default | **DELETED in v0.8.3** — migrated to default profile |
