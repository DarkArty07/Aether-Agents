...
The owner-authorized `1.0.0rc1` release-candidate objective (`oc_3397f9f05d780f8e@v1`) is one such bounded authorization: package `1.0.0rc1`, annotated tag and GitHub prerelease `v1.0.0-rc.1`, `release_impact=major` / `release_action=publish` / `release_channel=prerelease` — explicitly not stable `1.0.0`, not a PyPI publication and not a WSL2 qualification result. Issue #261 stays open with the stable, PyPI and WSL2 gates outstanding.
