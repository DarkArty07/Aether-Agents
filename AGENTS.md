...
The owner-authorized Morfeo MCP objective (#515) uses this local corrective `1.0.0rc12` compatibility bridge after rc11 safely moved the live installation off rc8. RC12 preserves rc11's historical emitted profile surface and schema-5 reader while adding the reviewed role-aware profile reader from #504, so it can accept a later target whose Morfeo bundle adds the canonical `plan` skill without itself emitting that skill. It still emits schema 4 and does not expose Morfeo MCP. The executable Hermes pin remains `aed6591a69f453a1867b73628603e7b53ba40ffc`. This bridge is local migration evidence only; it does not rewrite rc9/rc10/rc11 history, change Hermes, publish a tag, or qualify stable release.
