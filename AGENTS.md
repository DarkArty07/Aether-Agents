...
The owner-authorized Morfeo MCP objective (#515) uses this local corrective `1.0.0rc11` compatibility bridge after the reviewed rc9 source candidate proved non-installable from the live rc8 manager because rc9 also carried the later `plan` profile resource. RC11 is based on the exact rc8 product surface plus the reviewed schema-5 reader/`mcp` dependency-export bridge only. It still emits schema 4 and does not expose Morfeo MCP. The executable Hermes pin remains `aed6591a69f453a1867b73628603e7b53ba40ffc`. This bridge is local migration evidence only; it does not rewrite rc9/rc10 history, change Hermes, publish a tag, or qualify stable release.
