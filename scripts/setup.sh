...
install_hermes_agent() {
    ...
    if [ "$pip_exit" -ne 0 ]; then
        fail "pip install hermes-agent failed (exit ${pip_exit})"
        info "Retry with verbose output: pip install hermes-agent"
        exit "$pip_exit"
    fi
...
# DEPRECATED: wrappers no longer needed — PATH resolves to venv binary directly
...
#
#    # ── aether / hermes wrappers — point to hermes-agent binary ────────────
...
print_summary() {
    ...
    echo "     → See docs/guides/CONFIGURATION.md §6.1 for the full provider key list"
    echo ""
    ...
    if [ "${IS_WSL:-false}" = true ]; then
        ...
        if command -v update-desktop-database &>/dev/null || [ -n "${DISPLAY:-}${WAYLAND_DISPLAY:-}" ]; then
            wsl_lines="${wsl_lines}  • If using Windows terminals, update any .desktop shortcuts\n    to point to: ${HERMES_PYTHON}\n"
        fi

        if command -v nvidia-smi &>/dev/null; then
            wsl_lines="${wsl_lines}  • GPU access requires NVIDIA driver for WSL (2GB+)\n"
        fi
