...
"""
    ...
    write_json(
        evidence_dir / "fault-injection.json",
        {
            ...
            "active_hook": str(active),
            "known_good_hook": str(backup),
...
def _run_acceptance(
    ...
    (evidence_dir / f"{prefix}-acceptance.stdout").write_text(result.stdout, encoding="utf-8")
    (evidence_dir / f"{prefix}-acceptance.stderr").write_text(result.stderr, encoding="utf-8")
...
def live_run(
    ...
    *,
    ...
    (evidence / "morfeo-final.txt").write_text(morfeo_text + "\n", encoding="utf-8")
    _write_transcript(evidence / "owner-transcript.txt", turns)
