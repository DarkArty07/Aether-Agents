...
def snapshot_board(
    ...
    *,
    ...
    write_json(evidence_dir / "board-list.json", tasks)
    for task in tasks:
        task_id = task.get("id")
        ...
        write_json(evidence_dir / f"board-show-{task_id}.json", show)
        runs = show.get("runs", []) if isinstance(show, dict) else []
        write_json(evidence_dir / f"board-runs-{task_id}.json", runs)
    return tasks
