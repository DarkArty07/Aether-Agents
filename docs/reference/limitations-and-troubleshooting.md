...
| Release-candidate scope | Source `1.0.0rc13` / display `1.0.0-rc.13` is a forward candidate, not an active release. It emits release-lock schema 5, keeps the rc9/rc11/rc12 schema-4 and schema-5 readers, and serves Morfeo MCP on stdio or `127.0.0.1` Streamable HTTP. It does not open a public port, tunnel, or credential exchange. Source or local tags do not prove activation or agent behavior. Earlier tags remain immutable and published rc.1 remains rejected. This is not stable `1.0.0`, a package-index publication or WSL2 qualification. | Verify the disposable rc12→rc13 forward path, then use `aether doctor` and the managed cutover receipt for live state; never infer activation from source, push tags or activate rc.1. Keep #261 open. || State export | `uninstall --export` returns `EXPORT_NOT_IMPLEMENTED`. | Preserve state; do not claim an export occurred. |
