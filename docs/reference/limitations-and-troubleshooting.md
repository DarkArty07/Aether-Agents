...
| Release-candidate scope | Source `1.0.0rc16` / display `1.0.0-rc.16` is a candidate, not an active release. It binds maintained-fork Hermes at `58f8c37a49b341f25b8fdd6310542fe932031b8d` (digest `a2a9b374bd2022c7f96242b0ab2c95691119262c925389eb3820ca627c581144`), qualifies required HLP-428 with HLP-433 deferred, and uses the durable-history fallback for retained RC15 review guidance when the optional context count is absent. It emits release-lock schema 5, keeps schema-4/5 read compatibility, and serves Morfeo MCP on stdio or `127.0.0.1` Streamable HTTP. Source or local tags do not prove activation or agent behavior. Earlier tags remain immutable and published rc.1 remains rejected. This is not stable `1.0.0`, a package-index publication or WSL2 qualification. | Verify the isolated RC15→RC16 transition path, then use `aether doctor` and the managed cutover receipt for live state; never infer activation from source, push tags or activate rc.1. Keep #261 open. || State export | `uninstall --export` returns `EXPORT_NOT_IMPLEMENTED`. | Preserve state; do not claim an export occurred. |
