...
| Release-candidate scope | Local corrective bridge `1.0.0rc11` / `1.0.0-rc.11` preserves the rc8 profile-resource surface while reading release-lock schema 4 and schema 5 and still emitting schema 4. Morfeo MCP is not part of this bridge; it exists only to reach a later schema-5 target safely from the installed rc8 manager. Source or a local tag does not prove activation. Earlier tags remain immutable and published rc.1 remains rejected. This is not stable `1.0.0`, a package-index publication or WSL2 qualification. | Use the managed update path and `aether doctor`; never rewrite rc9/rc10 history or infer activation from source/tag state. |
