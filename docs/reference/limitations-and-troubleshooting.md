...
| Release-candidate scope | The current local-only candidate is package `1.0.0rc6` / display `1.0.0-rc.6` / annotated tag `v1.0.0-rc.6`; it is pre-stable, and the earlier `1.0.0rc1` / `v1.0.0-rc.1` milestone remains published from this repository at the tagged commit without being accepted for activation. Neither is stable `1.0.0`, a package-index publication or a WSL2 qualification result. | Treat `release_channel = prerelease` as a bounded milestone only; never read a publication as acceptance, do not activate the rejected rc.1, and keep issue #261 open with the stable, PyPI/OIDC and WSL2 gates outstanding. |
