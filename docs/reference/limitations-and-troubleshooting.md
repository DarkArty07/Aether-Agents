...
| Release-candidate scope | Local corrective bridge `1.0.0rc12` / `1.0.0-rc.12` preserves rc11's historical emitted profile-resource surface while reading release-lock schema 4/5 and accepting either the historical profile inventory or the reviewed target shape where Morfeo alone adds `plan`. Morfeo MCP is not part of this bridge. Source or a local tag does not prove activation. Earlier tags remain immutable and published rc.1 remains rejected. This is not stable `1.0.0`, a package-index publication or WSL2 qualification. | Use the managed update path and `aether doctor`; never rewrite rc9/rc10/rc11 history or infer activation from source/tag state. |
