...
| Release-candidate scope | Source `1.0.0rc7` / display `1.0.0-rc.7` defines a local-only compatibility bridge carrying the #497 updater repair with RC6-identical Morfeo package-owned resources. Its source or local tag does not prove activation; RC6 and earlier tags remain immutable and published rc.1 remains rejected. Neither is stable `1.0.0`, a package-index publication or a WSL2 qualification result. | Verify the selected version with `aether doctor` and the managed cutover receipt; qualify frozen RC6-to-bridge update/rollback in isolation before live activation. Do not confuse the bridge with activated #495 guidance, push tags or activate rc.1. Keep #261 open. |
