...
TESTS:
- python3 validate_m1_2.py — exit 0; all 15 validation checks passed deterministically
SMOKE:
...
- none
REMAINING RISKS:
- Orca CLI catalog metadata does not contain machine-readable output schemas, timeout contracts, or recovery contracts, and lacks public commands for Run cancel/close, Task cancel, and aggregate cleanup, requiring RETURN_TO_M0_DESIGN.
SCOPE CONFIRMATION:
- active task only
