...
v0.19.3 deterministic Gate B is frozen at `7e432f5`; its live Gate C remains blocked. Chris approved the v0.19.4 Gate A contract in `V0.19.4_GATE_A_PROPOSAL.md` on 2026-07-27.

The approved model uses one immutable contract with fixed A/B worker bindings, a verifier-owned digest-addressed snapshot and a hard `A CLOSED` prerequisite before B admission. The existing public `harmonia start` seam may accept the complete fixed two-task contract without selecting or relaying between tasks. Deterministic Gate B implementation is authorized. Harmonia remains default-off; live ACP, configuration changes, gateway restart, merge, tag, deployment and publication remain unauthorized.
