...
| ORCA-DEBT-014 | `worker-start` readiness acknowledges TUI/input admission rather than provider execution | Aether must compose public `worker-read` with task-scoped liveness markers | Orca exposes one public post-submit provider-working acknowledgement | MITIGATED IN AETHER; M5.4 ACCEPTED WITH COMPOSED LIVENESS GATE |
| ORCA-DEBT-015 | `worker-start` receipt omits terminal/worktree placement used by the initial adapter | First qualification path stopped before Aether Dispatch admission | Compose public `worker-start` identity with public `worker-show` placement | MITIGATED; 22 FOCUSED CONTRACTS PASS |
...
- 2026-08-09T16:40:29-06:00 — M5.4 liveness correction closed — Preserved the original timeout and each correction-path failure; added task-marker/public-transcript acknowledgement, bounded fail-closed diagnostics, one empty-Enter maximum without prompt resend, authoritative witnessed timing and exact `worker-read --limit` composition. Owner-authorized `codex update` moved the local CLI from 0.146.0 to 0.147.0 without reauthentication or credential mutation.
- 2026-08-09T16:40:29-06:00 — M5.4 ACCEPTED — The corrected isolated Orca 1.4.167/Codex 0.147.0 `gpt-5.6-terra` candidate passed UC-C05 and UC-C03 with two Dispatches before polling, two model-liveness markers, positive overlap, both frozen verifiers, two technical completions, two-component integration, semantic close, zero retries, USD 0 PAYG and zero survivors. Ruff, compileall, 22 focused tests and 198 repository tests passed. Canonical evidence: `M5_MODEL_LIVENESS_CORRECTED_EVIDENCE.json`.
