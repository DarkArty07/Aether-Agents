...
Harmonia and the coordination kernel may coordinate bounded execution. Olympus owns ACP process and session lifecycle. Daimons own specialist judgment only inside explicit task, authority, and evidence boundaries.
...
- **Etalides:** broad codebase or external research.
- **Daedalus:** UX, interaction, product flow, and prototypes.
- **Ictinus:** backend architecture, data, and scalability consultation.
- **Athena:** adversarial security and trust-boundary review.
- **Ariadna:** project continuity curation.
...
Use the configured and authorized coordination surface. If Harmonia is the required path and specialist work applies, use it without a hidden legacy fallback. A coordination failure is evidence to classify, not permission to create overlapping authority or writers.
