...
**Status:** APPROVED FOR PLANNING — IMPLEMENTATION NOT AUTHORIZED
...
**Objective:** make the existing R2–R7 kernel the mandatory authority for one clean, fixed-graph, default-off Snake pilot without PilotStore writes, dual authority, semantic normalization, or manual coordinator repair.
...
R8L  Legacy R8 evidence frozen (BLOCKED)
  |
  v
R9   Convergence contracts and RED composition matrix
  |
  v
R10  Durable workflow projection and budget authority
  |
  v
R11  Ledger-native dispatch, lease/fence, and reconciliation
  |
  v
...
**Status:** PLANNED / CODE NOT AUTHORIZED
...
## R12 — Trusted evidence and review
...
**Exit:** either evidence-backed semantic acceptance plus verified `CLOSED`, or an honest fail-closed terminal outcome. A blocked result remains valid evidence.
...
- three QA attempts for one security task are exhausted.
...
| Source/test implementation | BLOCKED pending plan approval |
| Fake-runtime deterministic tests | BLOCKED pending implementation approval |
