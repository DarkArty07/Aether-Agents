...
| **PD-61** | **Hermes delivery is reproducible and identified as Aether's maintained fork.** Public Aether distribution includes the qualified fork runtime, with source attribution, license notices, artifact checksums and provenance. The existing isolated-runtime boundary remains. Exact bundle composition, fork artifact naming and source-mode encoding require the productization design to be reconciled before implementation; this decision is not a publication or installation receipt. | Christopher revises the maintained-fork policy or the associated distribution and maintenance trade-off. |
