...
proof of agent compliance; the open-ended observation requirement in #317 was closed at
owner direction without claiming organic PASS, and unverified behavioral limits remain explicit.
...
**Owner amendment, 2026-09-07 — maintained Hermes fork:** PD-49, PD-61, PD-64 and PD-65 now reflect the owner's decision to maintain and distribute a long-lived fork, including separately scoped future frontend adaptations. The former upstream-by-default/transition-only policy is superseded. Constitution, role boundaries, privacy, release testing and protected effects are unchanged. Reconciliation of detailed A1 source modes and release machinery is pending design; no runtime upgrade, migration, fork publication or frontend implementation is performed by this amendment. Historical rationale is retained in `specs/001-aether-v1-productization/research.md`.
...
| **PD-21** | In the pipeline, Supervisor independently reviews requirements and Implementer work it did not author. PD-73 additionally permits an explicitly attributed bounded repair: verification of that repair is not independent review, and any contract-required independence remains applicable. This does not require another agent for every small adjustment. Direct PD-44 stewardship is selected only when independent review would not add proportionate value; it is not disguised pipeline work. | A separate reviewing capability proves necessary beyond the three roles, or direct-route defects show review cannot remain a proportional judgement. |
...
| **PD-49** | **Aether qualifies and distributes its maintained Hermes fork as the product runtime.** Every Aether release pins the exact public fork repository, tag, commit, source/artifact digest, Python compatibility, and Aether compatibility, and installs the runtime in isolation without replacing a user's personal Hermes installation. Upstream is an explicitly selected integration source, not an automatically adopted runtime. This source-policy amendment does not change the installed baseline or release-lock schema by itself. | Christopher revises the maintained-fork policy or the associated distribution and maintenance trade-off. |
...
| **PD-52 — amended by the owner (2026-09-23)** | **Aether begins with the owner's Git choice and a Morfeo conversation, not execution setup.** The owner enters an existing folder and explicitly runs `git init` for a new repository; `aether init` requires that Git root, binds its portable identity and one exact-path native Hermes Project, but creates no Git repository, remote, commit, constitution, root `AGENTS.md`, board, worktree, or worker. `aether` then opens Morfeo in that registered project even when the unborn repository has no `AGENTS.md`. Morfeo first inspects the project and converses with the owner about intent, principles, and the project's testing standard; afterward Morfeo autonomously performs the warranted scoped preparation and chooses direct work or a contract handoff. Only a concrete contract handoff provisions a project-isolated execution board/workspace; mere initialization or conversation does not. The owner does not select internal boards, agents, or commit mechanics. Brownfield governance and existing work remain preserved, project selection stays exact, and no project shares execution state with another. | The owner changes the explicit-Git/four-step onboarding experience, or verified isolation cannot defer board creation while preserving an exact contract-bound workspace. |
...
| **PD-61** | **Hermes delivery is reproducible and identified as Aether's maintained fork.** Public Aether distribution includes the qualified fork runtime, with source attribution, license notices, artifact checksums and provenance. The existing isolated-runtime boundary remains. Exact bundle composition, fork artifact naming and source-mode encoding require the productization design to be reconciled before implementation; this decision is not a publication or installation receipt. **Maintained-fork reconciliation (2026-09-15):** that reconciliation is recorded under Objective Contract `oc_3397f9f05d780f8e@v1` — the release lock carries `schema_version` 4 with `hermes.source_mode` `maintained_fork`, binding repository `https://github.com/DarkArty07/aether-hermes`, the exact fork commit, source-tree digest, Hermes version/tag, Python compatibility, artifact closure and provenance. | Christopher revises the maintained-fork policy or the associated distribution and maintenance trade-off. |
| **PD-62** | **Aether follows the Linux XDG boundary.** Non-secret configuration lives under `~/.config/aether`; versioned runtimes and profiles under `~/.local/share/aether`; local state, logs, and backups under `~/.local/state/aether`; replaceable downloads under `~/.cache/aether`. Project repositories contain only portable project identity and contract artifacts. Boards, credentials, runtime state, and workspaces are local and remain outside tracked project content. **XDG reconciliation (2026-09-15):** immutable release code and Graphify components live under the data root (`$XDG_DATA_HOME/aether/runtime`, `$XDG_DATA_HOME/aether/components`); the operational Hermes homes and every mutable session, board, credential, memory, observation, monitor, knowledge and project-state artifact live under the state root (`$XDG_STATE_HOME/aether/...`). The lifecycle reconciles to that installed layout instead of moving the live homes again, and rollback never rolls user state backward. | A supported Linux/WSL2 environment cannot provide the XDG mapping or Hermes requires a different isolation boundary. |
...
| **PD-64** | **Release qualification exercises the exact public Aether distribution with its pinned maintained Hermes fork.** The release-candidate path retains the accepted public installation path, provider independence, complete three-role workflow, native-Linux and WSL2 matrix, and state-preserving update/rollback checks. Credentials and model spend remain separately governed and never enter release artifacts. | Christopher revises the maintained-fork policy or the associated distribution and maintenance trade-off. |
| **PD-65** | **Aether maintains a long-lived Hermes fork `DarkArty07/aether-hermes` (maintained branch `aether-main`) as its product runtime boundary.** Owner-approved on 2026-09-07, replacing the transition-only policy. Aether owns maintenance of its documented runtime changes and may design product-specific core and frontend adaptations within separately authorized objectives. Upstream adoption occurs only when deliberately selected, with reconciliation and verification of Aether changes; returning to unmodified upstream is not a mandatory retirement goal. The transition starts from the actual local Hermes installation source and its identified local changes, not from the older remote fork main or a new upstream checkout. Reusing a hosting repository does not select its existing branch as the code baseline. Preserve an exact private source/build inventory first; classify and qualify changes before accepting them into public fork history. Unclassified changes are preserved, not silently discarded or published. The current runtime is not upgraded by this decision. Fork repository choice, integration workflow, versioning and distribution details remain under design; no specific frontend work is authorized yet. **Maintained-fork reconciliation (2026-09-15):** the repository, maintained branch `aether-main` and release-lock source-mode encoding (`schema_version` 4, `maintained_fork`) are settled by Objective Contract `oc_3397f9f05d780f8e@v1`; that objective replaces the manually copied runtime through the supported `aether update` lifecycle and activates the published release candidate locally. No frontend work is authorized by it. | Christopher revises the maintained-fork policy or the associated distribution and maintenance trade-off. |
...
| **PD-73** | **Delegate for economy, parallel execution and preserved coordination context, not rigid role separation.** Implementer owns substantial implementation and its reversible local judgement. From the first delivery, Supervisor may directly repair an understood, bounded defect that restores already-agreed behavior when doing so costs less remaining work and delay than another handoff, remains verifiable and does not monopolize supervision. This includes integration glue, but never new product intent, acceptance or shared-interface decisions. Attribute and verify the repair without claiming independent self-review. Ordinary review allows at most two returns per logical unit, with earlier stopping when progress or a better diagnosis is absent; exhaustion requires a bounded repair or an explicit recovery decision with originating Morfeo, never automatic acceptance or a silent reset. R7 owns the procedure; this is an instruction-level policy, not a new runtime gate. | Evidence about convergence, quality or Supervisor capacity, or an explicit owner decision, changes the repair boundary or initial return budget. |
