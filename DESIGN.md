...
**Status:** accepted current conceptual design through PD-70
...
| **PD-66** | **A guard is acceptable only when its precision is demonstrated over normal work as well as protected effects.** Aether MUST maintain positive cases that allow representative authorized Morfeo, Supervisor, and Implementer work; negative cases that block every protected effect; and an end-to-end pipeline that completes without guard-caused manual recovery. Every material false positive becomes a regression. If three focused corrections expose material false positives in distinct categories, incremental patching stops and the boundary is redesigned or replaced rather than preserving interruption by inertia. Structural incompatibilities SHOULD be rejected before execution through Aether-owned validation; runtime hooks remain a final defense for effects they can classify with high confidence. | Christopher changes the precision standard, or executed evidence supports a simpler boundary with equal authority and safety guarantees. |
| **PD-67** | **Morfeo may author canonical contracts with structured file tools either on the launcher-bound integration checkout or in the exact linked worktree of an active, board-verifiable Morfeo task.** Task-bound authorization requires matching task, task-owned run, assignee, active status, explicitly pinned board, `workspace_kind=worktree`, workspace, project root, and branch; missing or conflicting identity fails closed. Morfeo structured writes in either context require absolute target paths, and patch target extraction must remain parser-equivalent to the lock-selected Hermes version, including validation of both endpoints of a move; unrecognized operation headers fail closed. A clone, environment variables alone, or merely naming a branch never proves authorization. The isolated result receives independent review before protected integration, and this permission does not extend contract ownership to Supervisor or Implementer. Shell and code-execution tools remain part of Morfeo's broader bounded operational authority and are not misrepresented as being path-confined by this structured-file check. | Christopher withdraws task-bound canonical authoring, or evidence shows that the binding cannot be verified without unacceptable interruption or authority risk. |
