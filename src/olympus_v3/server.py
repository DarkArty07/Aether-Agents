...
async def _handle_aether_status(args: dict) -> list[mcp_types.TextContent]:
    ...
    try:
        ...
        try:
            ...
            # Counts
            # ⚡ Bolt: Combine multiple COUNT(*) queries into one using scalar subqueries to reduce connection overhead
...
async def _handle_aether_curate(args: dict) -> list[mcp_types.TextContent]:
    ...
    if schema_path.exists():
        try:
            # We check for existence but no longer read the entire file if we don't use it.
            pass
        except Exception:
            pass
