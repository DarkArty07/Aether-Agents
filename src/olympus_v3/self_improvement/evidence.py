...
def render_release_evidence(manifest: CycleManifest, ledger: SelfImprovementLedger) -> str:
    ...
    lines = [
        ...
        *_count_lines(counts["coordination_outcomes"]),
        f"- events with an uncertain durable effect: {counts['coordination_events_with_uncertain_effect']}",
        "",
        "## Measurement integrity",
        "",
        f"- Sessions without a clean baseline worktree: {counts['sessions_without_clean_baseline']}",
        f"- Sessions whose manifest changed mid-session: {counts['sessions_with_manifest_drift']}",
        "",
