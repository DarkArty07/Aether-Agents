...
class OlympusDB:
    ...
    async def get_session_progress(self, session_id: str) -> dict[str, Any]:
        ...
        # Combine multiple COUNT queries into a single query to reduce connection/I/O overhead
        cursor = await self._execute(
            """
            SELECT
        ...
        thoughts = counts_row[0] if counts_row else 0
        messages = counts_row[1] if counts_row else 0
        tool_calls_count = counts_row[2] if counts_row else 0
