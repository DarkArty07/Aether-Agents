...
class OlympusDB:
    ...
    async def get_session_progress(self, session_id: str) -> dict[str, Any]:
        ...
        # Combine independent aggregate queries into a single query using scalar subqueries
        ...
        row = await cursor.fetchone()
        if row:
            thoughts, messages, tool_calls_count, heartbeat_timestamp = row[0], row[1], row[2], row[3]
        else:
            thoughts, messages, tool_calls_count, heartbeat_timestamp = 0, 0, 0, None
