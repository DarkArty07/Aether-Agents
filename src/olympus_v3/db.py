...
class OlympusDB:
    ...
    async def get_session_progress(self, session_id: str) -> dict[str, Any]:
        ...
        # Counts
        # ⚡ Bolt: Combine COUNT queries into a single round-trip to reduce async/I/O overhead
        cursor = await self._execute(
            """
            SELECT
...
class OlympusDBSync:
    ...
    def get_session_progress(self, session_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Counts
            # ⚡ Bolt: Combine COUNT queries into a single round-trip to reduce SQLite connection/I/O overhead
            cursor = conn.execute(
                """
                SELECT
                    (SELECT COUNT(*) FROM turns WHERE session_id = ? AND role = 'assistant'),
