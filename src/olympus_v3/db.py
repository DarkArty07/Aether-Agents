...
class OlympusDB:
    ...
    async def get_session_progress(self, session_id: str) -> dict[str, Any]:
        ...
        # Batch counts into a single query to reduce connection overhead
        cursor = await self._execute(
            "SELECT "
        ...
        thoughts = counts[0] if counts else 0
        messages = counts[1] if counts else 0
        tool_calls_count = counts[2] if counts else 0
