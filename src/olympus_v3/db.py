...
class OlympusDB:
    ...
    async def get_session_progress(self, session_id: str) -> dict[str, Any]:
        ...
        # ⚡ Bolt: Batch independent aggregate queries to reduce SQLite roundtrips
        query = """
            SELECT
        ...
        """
        cursor = await self._execute(query, (session_id, session_id, session_id))
        row = await cursor.fetchone()
        thoughts = row[0]
        messages = row[1]
        ...
        try:
            ...
            query = """
                SELECT
                    ...
                    (SELECT COUNT(*) FROM tool_calls WHERE session_id = ?)
            """
            cursor = conn.execute(query, (session_id, session_id, session_id))
            row = cursor.fetchone()
            thoughts = row[0]
            messages = row[1]
            tool_calls_count = row[2]
