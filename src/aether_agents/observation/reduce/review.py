...
__all__ = ["apply_since", "build_review_brief"]
...
#: Change classes reported by ``--since`` (OBS-FR-069).
CHANGE_CLASSES = (
    "verdict",
    "next_gate",
    "required_work",
...
def apply_since(summary: dict[str, Any], previous: dict[str, Any] | None) -> dict[str, Any]:
    """Attach semantic-change classes relative to an explicitly selected prior summary.

    Token and tool-count growth alone is NOT a semantic change; a different
    ``fingerprint_key_id`` is a comparison boundary, not a configuration change. The caller
    is responsible for refusing incompatible summary schemas before calling this.
    """
    brief = dict(summary["review_brief"])
    if previous is None:
        return summary
    changed: list[str] = []
    if summary["review_brief"]["verdict"] != previous["review_brief"]["verdict"]:
        changed.append("verdict")
    if summary["review_brief"]["next_gate"] != previous["review_brief"]["next_gate"]:
        changed.append("next_gate")
    if _required_work_shape(summary) != _required_work_shape(previous):
        changed.append("required_work")
    if _acceptance_shape(summary) != _acceptance_shape(previous):
        changed.append("acceptance")
    if _anomaly_shape(summary) != _anomaly_shape(previous):
        changed.append("anomaly")
    if _ids(summary["bottlenecks"]) != _ids(previous["bottlenecks"]):
        changed.append("bottleneck")
    if _ids(summary["defect_attributions"]) != _ids(previous["defect_attributions"]):
        changed.append("defect")
    if _process_shape(summary) != _process_shape(previous):
        changed.append("process")
    if _comparable_configuration(summary) != _comparable_configuration(previous):
        changed.append("configuration")
    if _coverage_shape(summary) != _coverage_shape(previous):
        changed.append("coverage")

    brief["since_summary_id"] = previous["summary_id"]
    brief["change_classes"] = [c for c in CHANGE_CLASSES if c in changed]
    updated = dict(summary)
    updated["review_brief"] = brief
    return updated


def _required_work_shape(summary: dict[str, Any]) -> Any:
    return sorted(
        (u["task_ref"], u["task_status"], u["review_state"])
        for u in summary["work_graph"]["units"]
        if u["required"]
    )


def _acceptance_shape(summary: dict[str, Any]) -> Any:
    return sorted(
        (c["criterion_ref"], c["state"], bool(c["evidence_refs"]))
        for c in summary["acceptance"]["criteria"]
    )


def _anomaly_shape(summary: dict[str, Any]) -> Any:
    flow = summary["flow"]
    return (
        summary["runtime_state"]["anomalies"],
        flow["regressions"],
        flow["semantic_loops"],
        flow["unexplained_reversions"],
        sorted(f["code"] for f in summary["review_brief"]["findings"]),
    )


def _process_shape(summary: dict[str, Any]) -> Any:
    process = summary["process"]
    return (
        len(process["steps"]),
        len(process["waves"]),
        [r["trigger"] for r in process["rounds"]],
        process["critical_path"]["step_ids"],
    )


def _comparable_configuration(summary: dict[str, Any]) -> Any:
    """Only fingerprints inside one key epoch are equality-comparable (OBS-D-028)."""
    return sorted(
        (
            f["fingerprint_key_id"],
            f["scope"],
            f["participant_ref"],
            f["model"],
            f["provider"],
            f["system_prompt_fingerprint"],
            f["declared_toolset_fingerprint"],
        )
        for f in summary["configuration_fingerprints"]
    )


def _coverage_shape(summary: dict[str, Any]) -> Any:
    return (
        summary["coverage"]["complete"],
        sorted((g["class"], g["reason_code"]) for g in summary["coverage"]["gaps"]),
        summary["capability_evidence"]["surface_coverage"],
        summary["model_context_economics"]["token_coverage"],
    )


def _ids(records: list[dict[str, Any]]) -> Any:
    return sorted((r["attribution_id"], r["class"], r["provenance"]) for r in records)
