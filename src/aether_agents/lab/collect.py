"""Small, content-free evidence helpers for disposable E2E runs."""
...
def git_snapshot(repo: Path, *, env: Mapping[str, str] | None, log_path: Path) -> str:
    ...
    for label, argv in (
        ...
        returncodes[label] = result.returncode
    return json.dumps({"kind": "git_snapshot", "returncodes": returncodes}, sort_keys=True) + "\n"
...
def git_diff(repo: Path, *, env: Mapping[str, str] | None, log_path: Path) -> str:
    ...
    return json.dumps({"kind": "git_diff", "returncode": result.returncode}, sort_keys=True) + "\n"
