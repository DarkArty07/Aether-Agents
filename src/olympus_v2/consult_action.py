"""Olympus v2 Consult Action — handles the consult MCP tool using Pi Agent RPC.

Implements the consulting workflow where Daimons (Daedalus, Athena) act as
consultants: they review plans, provide enrichments, sign contracts, and
refuse tasks outside their scope.

Actions: start, run, sign, add_agent, status, complete.

The `run` and `sign` actions delegate to Daimons via the Pi Agent RPC mechanism
(spawn process → send prompt → poll loop → terminate), replacing the ACP
approach used in Olympus v1.
"""
...
logger = logging.getLogger("olympus_v2.consult_action")

# Poll/stall constants
POLL_INTERVAL = 15
...
# ---------------------------------------------------------------------------
# Pi RPC delegation — runs a prompt against a Daimon and waits for response
# ---------------------------------------------------------------------------
...
async def _delegate_to_agent(
    ...
    """Send a prompt to a Daimon via Pi Agent RPC, wait for completion, return response text.

    Uses the same session lifecycle as the v2 server delegate action:
    spawn → send prompt → poll loop → terminate.

    Args:
        adapter: The PiAdapter instance for managing Pi Agent processes.
        buffers: The global buffers dict for tracking session state.
        agent_name: Name of the Daimon to consult (must be in config).
        prompt: The consultation prompt to send.
        poll_interval: Seconds between poll iterations.
        timeout: Maximum seconds to wait for completion.

    Returns:
        The agent's response text as a string.

    Raises:
        ValueError: If the agent is not found in configuration.
        RuntimeError: If the agent times out.
    """
    # 1. Resolve agent config
    cfg = get_config()
    daimon_cfg = cfg.daimons.get(agent_name)
    if daimon_cfg is None:
        available = list(cfg.daimons.keys())
        raise ValueError(
            f"Unknown Pi-backed agent: {agent_name}. Available: {available}"
        )

    # 2. Spawn Pi Agent process
    logger.info(f"[consult] Spawning Pi Agent for {agent_name}")
    session = adapter.spawn_agent(daimon_cfg)
    session_id = session.session_id
    buffer = SessionBuffer(session_id=session_id)
    buffers[session_id] = buffer

    try:
        # 3. Wait for process initialization
        await asyncio.sleep(0.5)

        # Drain initial events (agent_start, etc.)
        ...
        # Drain stale events after prompt send (do NOT translate —
        # stale events can contaminate accumulated_text with prompt echo)
        await asyncio.sleep(0.5)
        stale_events = adapter.read_events(session_id)
        if stale_events:
            logger.info(
                f"[consult] Drained {len(stale_events)} stale events "
                f"for {agent_name} after prompt"
            )

        # Reset buffer to clean state before poll loop
        # (stale events drain or initial event drain may have left residue)
        ...
        # 5. Poll loop until done or timeout
        ...
        while True:
            ...
            logger.info(
                f"[consult] {agent_name} poll iteration={iteration}, "
                f"elapsed={elapsed:.1f}s, session_id={session_id}"
            )

            # Check for process death first
            if not adapter.is_process_alive(session_id):
                logger.warning(
                    f"[consult] {agent_name} process died unexpectedly, "
                    f"draining remaining events"
                )
                remaining = adapter.read_events(session_id) if session_id in adapter.sessions else []
                if remaining:
                    translate_events_batch(remaining, buffer)
                buffer.is_done = True
                if not buffer.final_response:
                    buffer.final_response = buffer.accumulated_text
                break

            # Read and translate new events
            ...
            if events:
                ...
                logger.info(
                    f"[consult] {agent_name} events breakdown: {event_types}"
                )
                translate_events_batch(events, buffer)

            logger.info(
                f"[consult] {agent_name} poll iter={iteration} "
                f"elapsed={elapsed:.1f}s events={len(events) if events else 0} "
                f"text={len(buffer.accumulated_text)}c reasoning={len(buffer.accumulated_reasoning)}c "
                f"last_turn={len(buffer.last_turn_response)}c turn={buffer.turn_count} "
                f"thoughts={buffer.thoughts_count} tool_calls={buffer.tool_calls_count} "
                f"done={buffer.is_done} stop={buffer.stop_reason or '-'}"
            )

            # Check if agent is done
            if buffer.is_done:
                logger.info(
                    f"[consult] {agent_name} completed in "
            ...
            if elapsed >= timeout:
                logger.error(
                    f"[consult] {agent_name} TIMED OUT after {timeout}s"
                )
                raise RuntimeError(
        ...
        # > final_response (from message_end) > accumulated_text (from current turn only)
        # > accumulated_reasoning (debugging only, never used as response)
        result = buffer.last_turn_response or buffer.final_response or buffer.accumulated_text or ""

        return result or "Error: Agent returned no response"

    finally:
        # 7. Clean up — terminate process and remove buffer
        try:
            adapter.terminate(session_id)
            buffers.pop(session_id, None)
        except Exception as e:
            logger.warning(
                f"[consult] Failed to terminate session {session_id}: {e}"
            )
...
async def _action_run(
    ...
    """Run a consultation with a specific agent."""
    if not session_id:
    ...
    # Delegate to the Daimon via Pi RPC
    try:
        raw_response = await _delegate_to_agent(adapter, buffers, agent, prompt_text)
    except Exception as e:
        logger.error(f"[consult] delegation failed for {agent}: {e}")
        return {"error": f"Failed to consult {agent}: {str(e)}"}
...
async def _action_sign(
    ...
    buffers: dict[str, SessionBuffer] | None = None,
    project_root: str | None = None,
) -> dict[str, Any]:
    """Get an agent to sign a final contract for specified tasks."""
    if not session_id:
    ...
    # Delegate to the Daimon via Pi RPC
    if adapter is None or buffers is None:
        return {"error": "Pi adapter not available for signing delegation"}

    try:
        raw_response = await _delegate_to_agent(adapter, buffers, agent, prompt_text)
    except Exception as e:
        logger.error(f"[consult] signing delegation failed for {agent}: {e}")
        return {"error": f"Failed to get signing from {agent}: {str(e)}"}
...
async def handle_consult_action(
    ...
    Args:
        ...
        adapter: The PiAdapter instance for Daimon communication.
        buffers: The global session buffers dict.
    ...
    try:
        ...
        return {"error": f"Internal error in consult/{action}: {str(e)}"}