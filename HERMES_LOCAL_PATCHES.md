...
- **Hunks HLP-211b exactos (checkout activo previo `0b288979e`, upstream inspeccionado `105b8650`):** `hermes_cli/kanban_db.py` amplía `VALID_ORIGIN_SIGNALS`; añade `_pending_flow_attentions`, `_wake_terminal_flow_controller` y `_resolve_flow_attention`; permite claim de una terminal card sólo con attention pendiente; integra wake en `block_task`; resuelve/reanuda el parent cuando el controller vuelve a `dependency`; inyecta el protocolo en `build_worker_context`. `tools/kanban_tools.py` conserva auto-subscription en raíces affinity no terminales y expone `recovery`. Tests añadidos en `tests/hermes_cli/test_kanban_session_affinity.py` y `tests/tools/test_kanban_tools.py`.
- **Tamaño HLP-211b antes de cualquier reformateo:** `hermes_cli/kanban_db.py` `+436/-53`, `tools/kanban_tools.py` `+90/-8`, `tests/hermes_cli/test_kanban_session_affinity.py` `+137/-0`, `tests/tools/test_kanban_tools.py` `+362/-0` frente al checkout ya parcheado; esos numstats incluyen otros cambios locales preexistentes en los mismos hotspots. La revisión final debe usar los hunks/símbolos anteriores, no atribuir el diff completo a HLP-211b.
...
- **Evidencia local:** HLP-211 base tenía `114 passed, 1 skipped`; E2E-16 y la sonda post-reinicio probaron reutilización exacta de sesión/workspace. HLP-211b registra RED previo (`flow_attention` ausente y `recovery` rechazado) y GREEN: `6 passed, 70 deselected` focalizados; suites afectadas `117 passed, 1 skipped`; Ruff check sin errores. El format-check global reporta deuda preexistente en los cuatro hotspots y no se usa como autorización para reformatearlos. Falta canary fresco post-reinicio que pruebe Implementer blocked → misma sesión Supervisor → parent resumed y Supervisor failed recovery → turno Morfeo sin segundo mensaje owner.
...
- **Estado de activación:** HLP-211 base `ACTIVE_LOCAL`. HLP-211b `CANDIDATE_NOT_RESTARTED`: archivos runtime editados y pruebas locales verdes, pero el gateway todavía conserva callbacks/código anteriores. La guardia negó editar `SOUL.md` pese a autorización explícita; no se rodeó. El worker context inyecta el protocolo de controller, por lo que la corrección runtime no depende de prompts, y el cambio documental de SOUL queda pendiente de un mecanismo autorizado separado.
