...
- **Primary active files (maintained fork `DarkArty07/aether-hermes`, branch `aether-main`):** `hermes_cli/kanban_db.py` and regression test suite `tests/hermes_cli/test_kanban_project_provenance.py`. (Note: `components` in the reconciliation fragment declares `hermes_cli/kanban_db.py` because the new test suite does not exist at pre-merge pin `aed6591a69f453a1867b73628603e7b53ba40ffc`).
