...
- **Upstream:** inspected at the selected public release `v2026.8.18` (commit `e624e9fde561e1add9388384012b295fde669ade`) and at `4f22543509d1b91dc45bcb369447126c5eb14fb7`. Both retain the same adapter shape: the per-item loop concatenates every `message` item (`agent/auxiliary_client.py:1839` and near line 1963 respectively), `finish_reason` is `"stop" if not tool_calls_raw else "tool_calls"` (`:1886` and near line 2010), and `_build_call_kwargs` (near line 8688) accepts no `tool_choice`. No equivalent upstream fix was found, so a downstream patch is required until an adopted release passes the recorded gate.
