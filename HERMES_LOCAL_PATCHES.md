...
- **Scope:** `hermes_cli/kanban_db.py` adds guarded controller requeue for pending attention, validates every recovery use against the current Project/flow/session/controller topology, rejects silent terminal-controller blocks, and routes the existing automatic terminal-failure boundary to one valid controller. The candidate reuses existing events and notification routing; it adds no gateway/TUI behavior, tool, CLI verb, table, status, service, watcher, or plugin.
- **Exact candidate provenance:** active `hermes-agent` `0.20.1` at committed `0b288979e2322c02ab42c05f1e183bb31cfa5aa9`; target pre-hashes were `58d97754650280db9fd1e7ad545b9af151b86d83d73d653dd84fb531f34a29b3` (`hermes_cli/kanban_db.py`) and `0acc517f72c670aa7c8e18b21ab5f9cc30e9b5ac1fbe37bd851bf23ef11d5ec7` (`tools/kanban_tools.py`). Candidate `kanban_db.py` hash is `5600a1b6f6e4f560dacebc8a5edc67505a66b7a287cd831c885b8b7f6be7f19f`; production patch delta is `+100/-28` (128 changed lines) in that file and zero tool production lines.
- **Portable artifact:** `patches/hermes/HLP-280-affinity-controller-requeue.patch`, SHA-256 `ebe680e932a55b6316dadae5b4886ad06f877b6bf68b6d5c494d81c7370a3c47`; forward/reverse apply and byte reconstruction passed, while activation and independent review remain pending.
- **Evidence:** isolated RED reproduced the four reviewer failures: external todo recovery, todo rediscovery, stale unblock, and stale claim. GREEN covers validated explicit unblock and rediscovery, sticky blocked preservation, unique attention/claim, malformed/ambiguous/cross-Project/flow/session isolation, stale-resolution rejection, terminal `flow_terminal`, below-threshold retry, and representative failure outcomes across 25 focused cases. Activation and live no-watcher canaries remain pending.
