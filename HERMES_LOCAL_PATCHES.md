...
- **Root cause and minimal semantics:** `_CodexCompletionsAdapter.create` reconstructs chat-compatible usage from Responses provider usage. When provider usage includes `output_tokens_details` (or `completion_tokens_details`) with `reasoning_tokens`, or a nested reasoning field in object or mapping form, the adapter preserves `output_tokens_details` on the chat-compatible `usage` object. `normalize_usage` in `agent/usage_pricing.py` reads this shape, preserving reasoning tokens through `_validate_llm_response` into `session_model_usage` without changing `input`, `output`, or `total` tokens and without double-counting reasoning into output or total. Explicit zero stays zero, absent detail stays zero/unknown, and malformed optional details do not raise or fabricate values.
- **Primary active files (maintained fork `DarkArty07/aether-hermes`, branch `fix/433-reasoning-usage`):** `agent/auxiliary_client.py` and regression test suite `tests/agent/test_auxiliary_client_responses_reasoning_433.py`. At the candidate pin `58f8c37a49b341f25b8fdd6310542fe932031b8d`, only `agent/auxiliary_client.py` exists, so the reconciliation fragment declares only that component; the new test module is introduced by this patch and is described in the artifact and retirement evidence.
- **Integration identity:** candidate fork commit `c31bd6ca149d6d23ebe5678b4a3d91aa17b5de91` on branch `fix/433-reasoning-usage` (tree `b1d86fe3c4a973e8c89b5d6aa5c5d4e96dcb89a5`), branching from base `58f8c37a49b341f25b8fdd6310542fe932031b8d` (tree `a93162c1a867202b03c12fa372c71029152fdcf7`). Fork merge and Aether integration are owned by downstream unit `INT-433`.
...
- **Rollback:** reverse only `HLP-433` (`git apply --check -R`), or revert the candidate fork commit on `fix/433-reasoning-usage` / `aether-main`. No database migration or irreversible schema change is involved.
...
- **Status:** `ACTIVE_LOCAL / UPSTREAM_MISSING`. Reviewed and approved in maintained-fork unit `HF-433`; portable artifact and reconciliation fragment prepared in Aether unit `AE-433`. Fork merge, Aether PR merge, and issue comment remain pending in `INT-433`.
