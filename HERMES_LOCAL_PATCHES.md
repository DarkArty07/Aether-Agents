...
| `HLP-263` | `#263` | los guards terminal y CLI directo usan titularidad real de gateway supervisado, no marcadores heredados | PR `NousResearch/hermes-agent#93267`, issue `#92560`, tip `aa9aaaa6cb31753c3b274db6825fbd0af5f27120` | `ACTIVE_LOCAL / UPSTREAM_VERIFIED` |
...
- **Backup y artefacto HLP-211b:** `.aether/backups/hlp248-flow-blocker-routing-20260829T193117-0600`; `SHA256SUMS` conserva los cuatro archivos previos y `LEDGER.sha256` el ledger previo. Parche portable versionado: `patches/hermes/HLP-211b-flow-blocker-routing.patch`, SHA-256 `7dceea9b9561c626fa6106f4bcd049592d9cb3627e2e0caed07a34df7d088bda`; `git apply --check` y comparación byte a byte contra los cuatro archivos activos pasaron sobre una reconstrucción del backup.
...
- **Estado de activación:** HLP-211/HLP-211b `ACTIVE_LOCAL_E2E_QUALIFIED`. Gateway Morfeo reiniciado el 2026-08-29 19:44 CST a PID `181495`; sonda post-reinicio verde. E2E-15A live `e2e-15-20260829-220207-c384343e` terminó `PASS` con `native_same_session_wake=true`, un solo owner submit, cuatro cards y acceptance verde. El canary blocker live `e2e-17-20260829-231107-c50462c6` produjo dos `flow_attention`, un `origin_signal=recovery`, misma sesión Supervisor generation 3, wake de la misma sesión Morfeo, hook Implementer restaurado, controller `blocked→completed` y verificación terminal exit 0; evidencia: `evidence/hlp211b-live-gate.json`. Su status agregado fue `FAIL` únicamente porque el acceptance del harness leyó el checkout raíz y no el worktree integrado; no se presenta ese agregado como PASS. `build_worker_context` hace que runtime no dependa de SOUL.
...
- **Extensión HLP-226b:** una card terminal Supervisor con afinidad comparte correctamente el worktree de la raíz y por eso persiste `workspace_kind=dir`, `workspace_path=.worktrees/<root-id>` y un task id distinto. El fallback anterior exigía `workspace_kind=worktree` y `workspace leaf == source task id`; descartaba esa fuente canónica y creaba rework `project_id=null`/`workspace_path=null`. HLP-226b acepta únicamente la raíz real indicada por el leaf cuando raíz y terminal coinciden en Project, assignee, flow y path; cualquier `dir` arbitrario continúa sin herencia.
- **Evidencia:** reproducción real `t_e729952b → t_21b8341a` produjo el huérfano runnable y reabrió #226. Regresión exacta RED `1 failed` (`project_id None`); GREEN `1 passed`; controles heredados `4 passed`; suites affinity/tools/worktree `80 passed`; `py_compile`, Ruff check y `git diff --check` verdes. La misma prueba incluye un control negativo donde un `dir` fuera de `.worktrees` conserva Project/path nulos.
- **Backup y artefacto HLP-226b:** backup previo `.aether/backups/hlp226b-affinity-terminal-20260830T144501-0600`; SHA-256 previos: `kanban_db.py` `dcb073c15f9e439a7f0a82f958e95f77e13831993e5f881514d4a024c82d9d0f`; test `90b0cc59e76c4727eb30aad28fde7c80ce134e1892bd48369f079b7481e5ac01`. Parche portable `patches/hermes/HLP-226b-affinity-terminal-project-inheritance.patch`, SHA-256 `a28fd10888932f421d32d41e1012ec7aad17280ae9e289c4d0329ff492f6c040`; `git apply --check` y reconstrucción byte a byte verdes.
- **Upstream:** `b9b5481d6236edb3ec8aae32cc4b5c661569b872` cubre sólo la fuente worktree directa. `NousResearch/hermes-agent@4f22543509d1b91dc45bcb369447126c5eb14fb7` no reconoce la card terminal affinity/`dir`; HLP-226b permanece local.
- **Estado de activación:** `ACTIVE_LOCAL + HLP-226b`; cada worker nuevo importa el handler/DB desde el checkout editable, por lo que no requiere reinicio del gateway.
- **Gate de retirada:** repetir ambos E2E con registro de Projects vacío: fuente worktree directa y fuente terminal affinity compartida; retirar sólo si ambos hijos conservan UUID, worktree y rama, el checkout real se materializa, un Project conflictivo se rechaza y un `dir` no canónico no se usa como fuente.
...
- **Activación y retirada:** Morfeo respaldó los tres archivos en `.aether/backups/hlp263-gateway-ownership-20260831T003539-0600`, verificó los hashes pre-parche, `git apply --check`, aplicó sólo HLP-263 y repitió `127 passed`, `3 passed` de ownership, Ruff, `py_compile` y `git diff --check`. El gateway se recargó de PID `475712` a `675455` y quedó `active`; un proceso CLI fresco con `_HERMES_GATEWAY=1` y `HERMES_GATEWAY_SESSION=1` ejecutó `gateway restart --help` con `rc=0` y mostró usage. Estado `ACTIVE_LOCAL / UPSTREAM_VERIFIED`. Para retirar al actualizar Hermes, la revisión objetivo debe contener equivalencia de `aa9aaaa6cb31753c3b274db6825fbd0af5f27120`; después ejecutar `git apply --check -R`, revertir únicamente estos hunks y repetir la misma matriz/canaria antes de recargar. Nunca restaurar archivos completos ni tocar el guard cron.
