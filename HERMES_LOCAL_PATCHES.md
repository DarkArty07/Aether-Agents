...
- **Local evidence:** fork candidate `622678efe9296a7485a817a7625eabfa00a65620` (base `9031bae0e8b0ab40c4fd7ba50c644972ff512611`).
...
- **Portable artifact:** `patches/hermes/HLP-426-terminal-worker-reap.patch`, SHA-256 `a7cd466cacd71dca2f091c34dc2fbb8d024991d727a7942f0cfc37eddf3a7679`. `git apply --check` passes on a freshly materialized tree at the exact base revision, and applying it reconstructs all three changed paths byte for byte against `622678efe9296a7485a817a7625eabfa00a65620`.
...
- **Activation and status:** source-only delivery in the maintained fork; the live runtime, its selector, launcher, service, profile, the published `v1.0.0-rc.1` tag/release and its assets were not touched by this unit, and no activation, reload or promotion was performed. Three integration-lane obligations are recorded rather than absorbed here: (a) this patch adds a test file that does not exist at the currently selected revision, so the structured reconciliation entry and aggregate regeneration must happen **after** the merge into `aether-main` (an entry declared today would be recorded as absent and refuse preparation instead of describing reality); (b) because this ledger file's digest is an input to the committed reconciliation aggregate, `--check` now reports `status: stale` in exactly one derived section (`source_ledger_sha256`) until that same regeneration runs — nothing else differs (`records: 29`, `refusing: []`); (c) the base-manifest allow-list of `.github/workflows/policy.yml` must gain this patch's path, which is the only unlisted tracked path the manifest check reports — maintenance of workflows is outside this unit's writable surface, so that wiring is routed to Supervisor. `ACTIVE_LOCAL / UPSTREAM_MISSING`.
