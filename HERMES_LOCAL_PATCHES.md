...
| `HLP-369` | `#369` | review readiness is distinct from final approval; native controller resume retains pending recovery attention | equivalent absent in inspected upstream `1675f1f2c25ce164f07c42e829f2c17a723db94f` and maintained-fork source | `LOCAL_CANARY_GREEN / FLOW_RECOVERY_PENDING` |
...
- **Activation:** fresh processes import the local repair; already-running callers retain old modules. No worker-killing restart. A one-shot native-tool recovery checks the actual controller transition. Local tests are not proof of MON-02 approval or full product delivery.
