...
| `HLP-211` | `#211` | afinidad opt-in reanuda una sesión worker exacta dentro de un Project/flow/perfil y un workspace canónico, con lease/generation fencing y retorno terminal al origen | Hermes `#75830`, `#59855`, `#68779`, `#71175`; PR `#75951` cubre sólo block→unblock de la misma card | `ACTIVE_LOCAL / UPSTREAM_PARTIAL` |
...
- **Evidencia local:** checkout Hermes limpio en `0b288979e` con `114 passed, 1 skipped` y Ruff verde; Aether focalizado con `65 passed` y Ruff verde. El canary live E2E-16 ejecutó procesos reales con Sol/Terra a través de Aether Router: la raíz Supervisor fue reclamada y reanudada, descompuso dos Implementers frescos, y una card Supervisor terminal reutilizó exactamente la sesión `20260828_144328_d7d5c0` y el workspace raíz. Los controles nativos cross-flow/Project/role, fencing, routing y revisión/integración quedaron `PASS` en evidencia schema-valid reconciliada. Tras reiniciar `hermes-gateway-morfeo.service` a PID `804025`, una sonda en procesos/SQLite temporales verificó raíz + terminal `done`, misma sesión `post-restart-supervisor-session`, generation `1→2`, workspace compartido y lease liberada.
...
- **Estado de activación:** `ACTIVE_LOCAL`; SOUL Morfeo/Supervisor/Implementer sincronizados byte a byte, wheel Aether instalado, gateway recargado el 2026-08-29 02:24 CST y sonda post-reinicio verde. Backup previo: `home/backups/hlp211-pre-activation-20260828T141725-0600`.
