...
- **Activation and status:** source-only delivery in the maintained fork; the live runtime, its selector, launcher, service, profile, the published `v1.0.0-rc.1` tag/release and its assets were not touched by this unit, and no activation, reload or promotion was performed. Two integration-lane obligations are recorded rather than absorbed here: (a) this patch adds a test file that does not exist at the currently selected revision, so the structured reconciliation entry and aggregate regeneration must happen **after** the merge into `aether-main` (an entry declared today would be recorded as absent and refuse preparation instead of describing reality); (b) the base-manifest allow-list of `.github/workflows/policy.yml` must gain this patch's path — maintenance of workflows is outside this unit's writable surface, so that wiring is routed to Supervisor. `ACTIVE_LOCAL / UPSTREAM_MISSING`.
