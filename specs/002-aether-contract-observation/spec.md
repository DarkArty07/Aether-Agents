...
| OBS-D-009 | Reuse Hermes shared-metrics tool category/outcome normalization and its registered toolset identity, but not its aggregate database as the contract journal. Resolving a tool's category must not import the full tool-discovery module during observer registration. | The taxonomy and lightweight native tool registry are already available in the selected runtime; the full discovery module can itself request plugin discovery, so importing it while another thread holds the plugin lock can deadlock agent startup. |
...
| OBS-D-032 | Observer registration and tool-category lookup MUST not acquire a plugin-discovery/import dependency cycle. The adapter uses the selected runtime's lightweight registered-tool lookup with the existing shared-metrics taxonomy, without importing the full `model_tools` discovery module during registration; lookup remains valid when later tools register. Missing native category capability is a visible coverage gap, not a fabricated exact classification or a reason to stop an agent. | An actual rc8 backend dump showed the MCP discovery thread holding the plugin lock while importing `model_tools`, whose importing builder thread waited for that same plugin lock. The installed runtime's `tools.registry` has a direct toolset lookup without importing `model_tools`; controlled concurrency and real MCP-enabled startup must still prove the fix. |
...
- **OBS-FR-087**: With the contract observer enabled and an MCP server configured, two concurrently starting discovery/build threads MUST not wait cyclically on plugin and module-import locks; agent readiness and the already accepted first user turn MUST complete when the MCP dependency is reachable, and an unavailable MCP dependency MUST degrade by the runtime's existing bounded/visible path rather than by an observer-induced hang. Tool categories produced after registration MUST match the selected Hermes registry plus shared-metrics taxonomy for registered tools; missing native capability remains explicitly incomplete. Qualification MUST exercise the actual selected runtime in isolation, not only a mocked import or a TUI frame.
