...
- **FR-804c**: The selected upstream `0.20.4` tag still persists a derived worktree branch and then spawns the stale claimed object (`hermes_cli/kanban_db.py:10230-10265`), so the first spawn can omit `HERMES_KANBAN_BRANCH`. Aether issue `#198` is closed for the loaded local repair, but that does not make the selected public tag conforming. Under the accepted maintained-fork release source — `DarkArty07/aether-hermes`, branch `aether-main`, selected by the release lock's `maintained_fork` source mode (`schema_version` 4) — the repair is carried as fork source, never as a replayed `.patch`, and it retires when the exact ready/review first-spawn matrix passes on an exactly released upstream artifact. No deliberate failed first attempt, database edit, global branch variable, hook weakening, or branchless directory worker is permitted.
...
- `kanban_db.py:10230-10265` confirms the selected tag still has the first-spawn branch propagation defect, which is why A1 carries the repair as maintained-fork source (`DarkArty07/aether-hermes`, branch `aether-main`) instead of relying on that public tag — never as a replayed `.patch` (maintained-fork reconciliation, 2026-09-15, Objective Contract `oc_3397f9f05d780f8e@v1`).
