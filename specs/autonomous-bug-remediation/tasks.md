...
t_1a2e74ee (Supervisor decomposition root)
    P0 → ABR-364 t_e4dac1b0  #364 contract path grammar (Implementer, Aether worktree)
    P0 → ABR-343 t_ec04ced6  #343 work-memory credential shapes (Implementer, Aether worktree)
    P0 → ABR-362 t_eb43b7bd  #362 independent review reclaim (Implementer, Aether worktree + nested fork worktree)
    P1 → ABR-315 t_174fa815  #315 authorized SOUL source edit (Implementer; fork nested worktree if source is fork)
    P1 → ABR-298 t_f30fd5f9  #298 placeholder-only docs guard (Implementer, Aether worktree)
    P1 → ABR-323 t_e6dcde87  #323 R0 scan ignore-boundary (Implementer, Aether worktree)
    P1 → ABR-284 t_773b8405  #284 brownfield .worktrees ignore (Implementer, Aether worktree)
    P2 → ABR-346 t_d7c3e3bb  #346 PR pagination / moving-head (Implementer, Aether worktree)
    P2 → ABR-META t_9f1c729d #306+#293 model metadata (Implementer, nested fork worktree)
    P2 → ABR-AUX t_57e549db  #296+#303 auxiliary client (Implementer, nested fork worktree)
    P2 → ABR-275 t_5edbf153  #275 local vision path (Implementer, nested fork worktree or evidence-only)
    P3 → ABR-360 t_694abc48  #360 exact-baseline checkout (Implementer, Aether worktree)
    P3 → ABR-357 t_d969ee72  #357 PluginContext harness (Implementer, Aether worktree)
    P3 → ABR-352 t_d9dff6d8  #352 concurrent lifecycle timeout (Implementer, Aether worktree)
    P3 → ABR-349 t_8e9a7bf7  #349 FTS projection trace (Implementer, nested fork worktree)
    P3 → ABR-329 t_20cd7628  #329 journal-storage concurrency (Implementer, Aether worktree)
    → same-card Supervisor review on each implementation unit
    → ABR-INT t_97e98c1d terminal integration/closeout (Supervisor, same flow, terminal=true)
```
