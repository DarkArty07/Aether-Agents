...
## 3.1 Objective Plans: route and continuity, not contract authority

- **FR-204b**: When meaningful planning or cross-session continuation is needed, Morfeo MUST maintain one stable `.aether/plans/<objective-slug>.md` in the explicitly resolved project. The plan is local and ignored by default; publication requires a separate portability/privacy decision. Do not initialize a project, choose a global latest plan, or migrate old plans merely to save continuity. A simple question or routine bounded adjustment requires no plan.
