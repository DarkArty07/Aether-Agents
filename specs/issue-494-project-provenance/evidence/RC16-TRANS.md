...
| --- | --- | --- | --- |
| `isolation` | Derived paths strictly under disposable work root; refusal of live overlap; scrubbed environment | 0 filesystem directory overlaps detected against configured live directories by path-listing; the isolation scenario checks its 10 listed derived paths under the temp work root, while the preflight resolves 15 derived destinations including candidate/fork/staging roots and refuses an escape before mutation; service boundary positively confined under fixed system temp roots (/tmp, /var/tmp, /private/tmp, /private/var/tmp); operator environment variables scrubbed | PASS |
