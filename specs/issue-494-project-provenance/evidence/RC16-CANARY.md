...
Note on the live-board evidence: an earlier draft of this file claimed all 99 board hashes were
compared byte-identical before and after. That comparison is withdrawn as invalid — the
fingerprint helper wrote to the same path it had read the pre-state from, so its "post" reading
was not an independent second measurement. The three independent checks recorded above (stable
structural totals, exact write attribution by mtime across all 99 boards, and the pinned
disposable board routing) support the preservation claim as stated here and no more.
