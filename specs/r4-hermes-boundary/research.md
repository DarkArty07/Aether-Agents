...
**Superseding note (2026-09-15, maintained-fork reconciliation under Objective Contract `oc_3397f9f05d780f8e@v1`).** The decision above is retained as the historical rationale for the six accepted changes recorded in this section; its mode selection is superseded, and this note supersedes the release-mode wording used elsewhere in this section. The executable release source is the maintained fork `DarkArty07/aether-hermes`, branch `aether-main`, bound by release-lock `schema_version` 4 source mode `maintained_fork` (repository, branch, exact commit, source-tree digest, artifact closure and provenance), and the retired `transitional_fork` mode is refused for new preparation. The changes are carried as fork source, never as replayed `.patch` files; `.patch` and HLP records remain audit/reconstruction evidence. Each carried change keeps the guarantee, upstream disposition, qualification evidence and behavior-based retirement condition recorded in the table above. Retiring the fork is no longer a mandatory goal: upstream is adopted only when deliberately selected, with the Aether changes reconciled and verified.
