...
- **FR-703**: Supervisor does not own feature implementation. Under PD-73, from the first delivery it MAY directly repair an understood, bounded defect that restores already-agreed behavior, including integration glue, when the remaining work and verification are more economical than another handoff and preserve its coordination capacity. It MUST NOT invent product behavior, acceptance, shared-interface decisions or scope. Substantial implementation remains delegated within the review budget; a repair that grows beyond these conditions requires reconsidering the route rather than absorbing unlimited implementation. Size in lines is not the criterion.
...
- **FR-714b**: All same-flow Supervisor phases MUST reuse one exact Hermes session and one canonical Supervisor workspace for conversation identity, prompt continuity and flow fencing. Separate operating-system processes resume that session; they MUST NOT create a fresh task-keyed Supervisor worktree for review or integration. During same-card review the implementation card and its candidate worktree remain the reviewed task/file surface; borrowing the Supervisor session MUST NOT persist Supervisor affinity onto the Implementer card or replace that candidate workspace.
...
- **FR-714d**: The runtime MUST fence the binding by board, Project, flow, profile, workspace, generation and lease. Resume MUST use the exact stored session with `--no-restore-cwd --in <canonical-workspace>` and reject missing, closed, corrupt, cross-profile, cross-Project, cross-flow, or stale-generation state. When review reads another candidate workspace, session/prompt identity remains canonical while task/file tools stay pinned to the candidate; this split MUST NOT mutate the stored session prompt merely because the reviewed worktree differs.
...
The two-return policy below is the owner's initial, uncalibrated setting, not a measured optimum or a runtime-enforced limit.
...
- **FR-734**: Supervisor MUST independently review the requirements and Implementer work it did not author (R3-FR-318). For a PD-73 bounded repair, it MUST distinguish the Implementer candidate, its own correction and the verified final candidate. Its own verification is not independent review; any applicable requirement for an independent reviewer of that repair remains in force.
...
- **FR-735a**: An opted-in Aether same-flow review MUST resume the existing Supervisor session and its canonical Aether review procedure without automatically stacking the generic bundled `sdlc-review` procedure or Implementer-pinned skills/model overrides from the same card. This is an Aether flow behavior only; generic Hermes review outside a verified Aether collaboration root retains its existing fresh-session/bundled-review behavior.
...
- **FR-736b**: Supervisor MUST own review convergence, not merely successive defect discovery. Repeated failure classes, equivalent cases exposed by successive fixes, or growing repair complexity without closing the obligation require a common-cause assessment before another piecemeal return. Consolidate findings against existing requirements and distinguish mandatory defects from optional improvements. Choose acceptance, a PD-73 direct repair, justified delegated rework, or recovery; an unsuitable or missing material design, scope, interface, guarantee or authority decision returns to Morfeo through existing collaboration. Preserve the candidate and evidence. Do not add universal guarantees, waive existing obligations, impose automatic approval after a round count, or interrupt healthy independent work. The canonical skill owns the detailed procedure; no new engine, form or judge is required.
- **FR-736c**: Every blocking review finding MUST remain traceable to a current acceptance/preservation obligation and to evidence from the artifact actually being cited. Source coordinates such as code line numbers MUST be verified against the exact Git/raw artifact whose coordinates are reported; a normalized, rendered or transformed copy MAY support content discovery but MUST NOT be presented as coordinate evidence for the original artifact.
- **FR-736d**: Re-review MUST begin from the delta between the previously reviewed candidate and the new candidate, then re-check the affected obligations, mandatory controls and regressions whose validity that delta can change. Evidence already established against an unchanged candidate/environment MAY be reused; repeating a broad verification requires a stated invalidation/risk reason rather than the ordinal review round alone. This never permits a real new defect or required integration gate to be skipped.
- **FR-736e**: Supervisor MUST allow at most two ordinary review returns per logical unit, not two cards or execution attempts. Consult existing durable history and record `Return 1/2` or `Return 2/2` with consolidated findings in the existing handoff; do not infer a complete count from a truncated history. Stop earlier without verifiable progress or a concretely improved diagnosis. After two returns, accept a supported result, make a genuinely bounded repair, or consult originating Morfeo with evidence and a recovery proposal; do not issue a third ordinary return, approve defects or take over substantial implementation. Sessions, compactations, replacement cards, changed implementers and contract successors MUST NOT silently reset the history. Any further recovery requires a justified change of approach and explicit budget within existing authority, not an automatic fresh loop. This is an instruction-level policy using existing history and collaboration: no new counter, schema, hook, runtime gate or workflow engine is required, and technical prevention of a third return is not claimed. Existing stricter contract limits remain applicable.
...
- **FR-737c**: On the maintained Aether runtime, same-card review continuity is derived only from a verified opted-in Aether collaboration root whose persisted contract/Project binding matches the board/task and whose same-profile affinity ancestry resolves to one existing Supervisor session. A native opt-in MAY retain an optional Aether-project snapshot only when the board, task and contract already corroborate it; historical events remain unchanged. Review examines ancestral native opt-ins before deciding the flow is generic. A board key alone never opts a task into Aether. A persisted snapshot, a complete corroborating board with an opt-in, or a corroborated legacy opt-in/board pair recognizes an Aether flow; once recognized, a missing, ambiguous or mismatched root/identity MUST fail through the existing lifecycle rather than create a new session. The review lease is ephemeral: the implementation card keeps `session_affinity=NULL`, its candidate worktree and its Implementer-phase configuration for possible rework. Legacy exact-flow cycles without the snapshot remain supported. If all relevant provenance and ancestry were erased, identity cannot be inferred and this requirement creates no magical fail-closed classification; an arbitrary board key remains generic.
...
- **SC-707**: No role represents verification of its own output as independent pipeline review. Supervisor's PD-73 bounded repairs retain explicit authorship and verification, without waiving any applicable independent-review requirement. Direct PD-44 verification is likewise not represented as independent pipeline review.
...
- **SC-710**: Guidance and focused resource checks distinguish a direct bounded repair restoring agreed behavior from substantial implementation, preserve attribution and require at most two ordinary review returns with justified recovery. These checks establish instructional consistency, not model compliance, savings or live adoption; any such claims require observed evidence.
