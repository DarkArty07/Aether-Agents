...
- **FR-737c**: On the maintained Aether runtime, same-card review continuity is derived only from a verified opted-in Aether collaboration root whose persisted contract/Project binding matches the board/task and whose same-profile affinity ancestry resolves to one existing Supervisor session. The review lease is ephemeral: the implementation card keeps `session_affinity=NULL`, its candidate worktree and its Implementer-phase configuration for possible rework. Missing, ambiguous or mismatched Aether identity MUST NOT be guessed from affinity ancestry alone; generic Hermes review remains generic.
