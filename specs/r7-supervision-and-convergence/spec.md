...
- **FR-736b**: Supervisor MUST own review convergence, not merely successive defect discovery. Repeated failure classes, equivalent cases exposed by successive fixes, or growing repair complexity without closing the obligation require a common-cause assessment before another piecemeal return. Consolidate findings against existing requirements and distinguish mandatory defects from optional improvements. A coherent bounded implementation correction stays with Implementer; an unsuitable or missing material design, scope, interface, guarantee or authority decision returns to Morfeo through the existing lifecycle. Preserve the candidate and evidence. Do not add universal guarantees, waive existing obligations, impose automatic approval after a round count, or interrupt healthy independent work. The canonical skill owns the detailed procedure; no new engine, form or judge is required.
