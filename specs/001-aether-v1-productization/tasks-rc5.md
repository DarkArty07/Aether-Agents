...
| Live unit state | `~/.config/systemd/user/hermes-gateway-morfeo.service` is already a **Hermes-generated** unit: `Description=Hermes Agent Gateway…`, `ExecStart=/home/darkarty/.local/share/aether/runtime/current/venv/bin/python -m hermes_cli.main --profile morfeo gateway run`, `WorkingDirectory=/home/darkarty/.local/state/aether/hermes/profiles/morfeo`, `Environment="VIRTUAL_ENV=…/runtime/current/venv"`, `Environment="HERMES_HOME=…/profiles/morfeo"`, WSL interop PATH entries, **no** `HERMES_TUI_DIR`. This is the observed ownership drift the contract resolves: rc4's doctor demands the Aether-owned TUI selector line that the gateway's own refresh removes |
...
| Preserved residue | Owner primary checkout `/home/darkarty/Desktop/agentes/aether` is clean at `e1c5f9a4`; retained `$XDG_STATE_HOME/aether/rc3-preflight`, `rc4-bootstrap*`, `recovery`, `diagnostics`, `hermes-preseed-partial-*` and the retained rc3 source quarantine stay untouched; unrelated units (`hermes-gateway-hestia`, `aether-router*`, `prometeo-*`) are never touched |
...
9. Environment: run `uv run --frozen …` from the unit's own worktree so it provisions
   ...
   the owner's primary checkout and never write `/home/darkarty/Desktop/agentes/aether`
