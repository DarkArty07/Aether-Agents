...
Delegated clarification for U1: preserve the existing normalization of a non-empty relative `--project PATH` against the invocation cwd, followed by exact marker/registry/conflict validation. Only the environment path selector `AETHER_PROJECT_ROOT` requires an absolute, non-empty value; `AETHER_PROJECT_ID` remains a canonical UUID consistent with the selection. The assumption is the already inspected and tested resolver behavior, not a new project-selection feature. The broader phrase "empty or relative identity" introduced by the documentation candidate must not prohibit explicit relative paths. A1-FR-043 and the CLI contract own this precise split; reconcile their wording without changing implementation or the finalized Objective Contract.
...
Delegated packaging disposition for the configured-cwd check: preserve the current manager/wheel dependency closure for this correction. The inspected rc5 manager requires exact wheel `Requires-Dist` equality with its locked `jsonschema` dependency and has no YAML interpreter; adding PyYAML to the incoming wheel or manager closure would invalidate the required first hop. The selected Hermes runtime already carries locked PyYAML (6.0.3 in the inspected fork closure). Use that target runtime's real YAML grammar through a bounded, isolated, fixed-operation read-only subprocess, reusing the selected lexical-venv provenance check. Do not import Hermes or require YAML inside the manager. The child may return only the needed cwd interpretation/status, never the full profile, secret values or raw parser exceptions; no config migration, plugin discovery, service/model request or write is part of the probe. `--json` remains non-mutating, and an unavailable interpreter must produce a bounded refusal rather than verified absence. Preserve exact project/conflict checks and the existing malformed-document disposition; do not rebuild another line parser. Qualification must use a manager installed from the wheel's declared dependencies alone and a distinct target runtime with its declared locked closure. Installing an undeclared YAML package beside the wheel in the manager fixture is not proof of supported installation. Helper shape and reuse remain local implementation choices; this resolves the packaging question under U1 without changing the Objective Contract, frozen releases or fork.
