...
**Contract ID**: A1
**Status**: ready for Supervisor executability analysis
**Accepted product decisions**: `DESIGN.md` PD-01 through PD-64
...
**Depends on**: `DESIGN.md`, R0-R13, the current public Aether repository, the public `DarkArty07/hermes-agent` fork, Hermes Agent upstream, PyPI, uv, and GitHub Actions
...
The public product is a packaged Python CLI named `aether-agents`, exposing the `aether` executable. It manages a qualified public downstream Hermes runtime without replacing a user's personal Hermes installation. Aether 1.0 is complete only after the exact public release candidate passes deterministic and live qualification through the same installation path offered to users.
...
- public, minimal, qualified downstream Hermes releases;
...
- reconciliation of every canonical or derived artifact invalidated by PD-48 through PD-64.
...
A Linux or WSL2 user installs `aether-agents` from PyPI with uv and runs guided setup. Aether verifies and installs the exact downstream Hermes artifact locked to that Aether release, creates the three portable profiles, and leaves any existing Hermes installation untouched.
...
3. Given a tampered or mismatched downstream artifact, when setup verifies the lock, then installation fails closed before executing or activating the artifact.
...
1. Given the RC's exact PyPI and downstream artifacts, when deterministic qualification runs, then every required test, package, manifest, provenance, documentation, secret, and clean-install check passes against those artifacts rather than a source-tree shortcut.
...
- **A1-FR-008**: Aether MUST remain MIT licensed and MUST preserve Hermes upstream license and attribution in the downstream source and release materials.

### 5.2 Qualified downstream Hermes

- **A1-FR-009**: Aether MUST consume Hermes through the public `DarkArty07/hermes-agent` downstream.
- **A1-FR-010**: The downstream MUST carry only patches required by an accepted Aether guarantee or a bounded compatibility/security repair.
- **A1-FR-011**: Every downstream-only patch MUST have a durable ledger entry containing purpose, upstream base, affected guarantee, verification, upstream issue/PR when applicable, and retirement condition.
- **A1-FR-012**: Generally applicable fixes MUST continue to be proposed upstream; an upstream equivalent MUST retire the downstream patch only after parity and regression qualification.
- **A1-FR-013**: Upstream adoption MUST be explicit and release-bound; no job, startup path, or package update MAY merge or install mutable upstream `main` automatically.
- **A1-FR-014**: Each Aether release MUST pin one exact downstream tag and commit derived from one recorded stable upstream tag and commit.
- **A1-FR-015**: The downstream CI MUST build the original `hermes-agent` wheel and source archive and publish them as GitHub Release assets with SHA-256 checksums and provenance.
- **A1-FR-016**: Aether MUST NOT upload the renamed downstream to PyPI or vendor Hermes source into the Aether package.
- **A1-FR-017**: The Aether release lock MUST bind Aether version, Python version range, upstream and downstream refs, artifact URLs, filenames, SHA-256 values, provenance URLs, and the profile-policy bundle digest.
...
- **A1-FR-020**: The six currently open upstream patch lines recorded in `HERMES_LOCAL_PATCHES.md` MUST be reconciled against the selected stable upstream baseline rather than copied blindly.
...
- **A1-FR-072**: The shared pre-tool policy MUST remain fail-closed for secrets, credentials, cross-role boundaries, and protected effects while permitting the accepted role capabilities.
...
- **A1-FR-084**: The release candidate MUST be installed from PyPI and MUST consume the exact public downstream GitHub Release artifact.
...
- **A1-FR-091**: Release qualification MUST reproduce and prevent the observed policy false positive in which read-only validation commands were classified as attempts to stop or restart the gateway. A denial remains authoritative during development; the correction requires an explicit regression test rather than a tool workaround.
...
- **A1-SC-008**: Built wheel, sdist, profile bundle, docs, release lock, and downstream release metadata contain zero detected secret/private-runtime material.
...
- **A1-SC-013**: Read-only contract/package validation executes without a false gateway-lifecycle denial, while real attempts to stop or restart the protected live gateway remain denied in the disallowed context.

## 9. Known assumptions and limitations

- The exact downstream upstream-base tag is selected and frozen during implementation from a current stable Hermes release; mutable upstream `main` is never a release base.
...
PD-48 through PD-64 supersede older text that described Aether as a private single-profile configuration layered on unmodified Hermes. Before implementation closes, the owning and derived artifacts MUST be reconciled. At minimum:

- R4: replace the no-fork rule with the qualified-downstream boundary;
