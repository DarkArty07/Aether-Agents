...
## 2. Packaged role closure and immutable readback (PS-006–008)
...
Expected tests: wheel and sdist carry byte-identical sanitized `skills/plan/SKILL.md`; candidate wheel digest equals its per-role bundle digest; fresh disposable Morfeo home has the canonical `plan`, other two role homes do not. Historical authenticated release bundles retain the old exact shape and hashes; forged/missing/extra role entries, symlinks, unsafe permissions and resource drift still fail. Add specific cases for a same-name private learned skill at the exact canonical path (backed up on adoption), a nested learned skill (preserved), simulated transition failure (prior bytes/selector restored), old→compatible→new qualification and new→old rollback (managed plan removed without touching the learned skill). A frozen predecessor's direct rejection of a new-shape wheel is an **expected incompatibility observation**, not a passing upgrade: an isolated compatible path must be demonstrated or the release remains blocked.
