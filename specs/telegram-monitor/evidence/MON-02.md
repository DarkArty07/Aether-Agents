...
- Reads canonical execution-board metadata and requires the canonical board slug,
  portable Aether project, native Project binding, Objective Contract ID/version, and a
  readable finalized contract with exact `created_in_session`/`finalized_in_session`
  provenance. Supports the native metadata keys `aether_project_id`,
