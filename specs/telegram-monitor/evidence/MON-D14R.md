...
| --- | --- | --- | --- |
| `test_d14r_stale_mapping_reproduction_refuses_at_preflight_before_lab_created` | FAIL (chain-driver return code 3; pytest exit 1) | PASS (chain-driver return code 0; pytest exit 0) | Pre-correction: preflight passed (`phase1_problems: []`), laboratory created on disk (`created: true`), in-lab gate passed (`gate_problems: []`, masked by `cron`), fixture child failed with `QualificationError: the synthetic laboratory scope could not be materialized through the shipped writers`. Corrected: refuses fail-closed at read-only preflight (`phase1_problems: ["provisioned-fixture-imports: ModuleNotFoundError", "provisioned-writer-interface-missing...", "provisioned-writer-artifact-missing..."]`), `created: false`, zero laboratory root created on disk. |
| `test_d14r_in_lab_gate_exercises_fixture_import_order_unmasked_by_cron` | FAIL (pytest exit 1) | PASS (pytest exit 0) | Pre-correction: in-lab gate probe reported `problems: []` because earlier `from cron.scheduler_provider import InProcessCronScheduler` bootstrapped `sys.path`, masking the stale mapping (observed failure: `assert any("fixture-imports" in p for p in problems)`). Corrected: in-lab gate executes fixture chain first, detecting `fixture-imports: ModuleNotFoundError` and missing writer interfaces unmasked by cron. |
| `test_d14r_preflight_refuses_missing_hermes_state_writer_surface` | FAIL (pytest exit 1) | PASS (pytest exit 0) | Pre-correction: `phase1_problems == []` because preflight did not validate `hermes_state` writer surface when `lab_root=None` (observed diff: left empty, right containing `provisioned-writer-interface-missing:hermes_state.SessionDB.create_session`). Corrected: `phase1_problems` includes `provisioned-writer-interface-missing:hermes_state.SessionDB.create_session` before lab creation. |
| `test_d14r_native_probe_body_executes_fixture_imports_before_cron_and_gateway` | FAIL (pytest exit 1) | PASS (pytest exit 0) | Pre-correction: the base `_LAB_NATIVE_PROBE` lacks the fixture chain — only the first chain statement is present there, and the run stops at the first missing statement, `AssertionError: from aether_agents.monitor.store import MonitorStore missing from _LAB_NATIVE_PROBE` (at base, `from hermes_state import SessionDB` occurs only in `_LAB_FIXTURE_PROBE`). Corrected: probe body executes fixture import chain in identical order to `_LAB_FIXTURE_PROBE` before `cron` or `gateway` imports. |
| `test_d14r_preserved_v3_failed_receipt_and_console_log_unchanged` | PASS (pytest exit 0) | PASS (pytest exit 0) | Preservation-only check: preserved v3 receipt digest `c23096fe...` and console log digest `a6046560...` remain byte-identical on both base and candidate. |

**Exit-code conventions (re-measured for this correction, 2026-09-10, on the candidate and on a scratch base worktree at `cf8cf3de` + this test file):** `pytest exit` is the exit status of `uv run --frozen python scripts/run_tests.py -- -q <node id>` run once per node on each side; `chain-driver return code` is the exit status of the `_CHAIN_DRIVER` child process that the first test spawns (the test asserts it is 0; a run that refuses at preflight prints its facts and exits 0, a run whose phase raises exits 3). Observed per-row: pre-correction row 1 driver return code 3 / pytest exit 1; corrected row 1 driver return code 0 / pytest exit 0; all other rows re-measured with the pytest exit statuses shown above.
