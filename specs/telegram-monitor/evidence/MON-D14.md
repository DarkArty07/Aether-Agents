...
1. **Profile resolution mismatch:**
   ...
   Under this installation's profile-scoped runtime, `HERMES_HOME` points directly to the Morfeo profile (`.../home/profiles/morfeo`), which caused the path to resolve to `.../home/profiles/morfeo/profiles/morfeo`. That path does not exist, causing `config-unreadable: FileNotFoundError` in `_lab_config` and raising bounded refusal `lab-config` before the attempt boundary.

   *Command executed:*
   ```bash
   env HERMES_HOME="/home/darkarty/Desktop/agentes/aether/home/profiles/morfeo" \
   ...
   ```
   *Observed output:*
   `/home/darkarty/Desktop/agentes/aether/home/profiles/morfeo/profiles/morfeo` (does not exist; `exists() == False`).
...
2. **Provisioned reference destination missing:**
   ...
   *Command executed on base behavior:*
   ```bash
   env -i PATH="$PATH" HOME="$HOME" HERMES_HOME="/home/darkarty/Desktop/agentes/aether/home/profiles/morfeo" \
     /home/darkarty/Desktop/agentes/aether/home/.venv-hermes/bin/python -c '
...
1. **`_normalize_profile_home`:**
   - Normalizes either the multi-profile installation root (`.../home`) or the exact profile home (`.../home/profiles/morfeo`) to the exact canonical Morfeo profile directory.
...
| --- | --- | --- | --- |
| D14 HERMES_HOME normalization (both shapes) | `test_d14_hermes_home_both_shapes_normalize_to_same_profile` | PASS: Both multi-profile root and exact profile resolve to the same canonical directory and digest (`de710532...`); classes recorded as `multi-profile-root` and `exact-profile` | `tests/test_telegram_monitor_cli_plugin.py:5858` |
| D14 Missing candidate refusal | `test_d14_normalize_profile_refuses_missing_and_wrong_candidates` | PASS: Nonexistent candidate raises `QualificationError("lab-profile")` before any effect | `tests/test_telegram_monitor_cli_plugin.py:5876` |
| D14 Wrong-named candidate refusal | `test_d14_normalize_profile_refuses_missing_and_wrong_candidates` | PASS: Candidate named "implementer" without `profiles/morfeo` raises `QualificationError("lab-profile")` | `tests/test_telegram_monitor_cli_plugin.py:5884` |
| D14 Ambiguous candidate refusal | `test_d14_normalize_profile_refuses_ambiguous_and_symlinked_candidates` | PASS: Directory named "morfeo" containing a `profiles/morfeo` child raises `QualificationError("lab-profile")` | `tests/test_telegram_monitor_cli_plugin.py:5892` |
| D14 Symlinked candidate refusal | `test_d14_normalize_profile_refuses_ambiguous_and_symlinked_candidates` | PASS: Symlinked profile directory raises `QualificationError("lab-profile")` | `tests/test_telegram_monitor_cli_plugin.py:5906` |
| D14 Missing / symlinked config refusal | `test_d14_normalize_profile_refuses_missing_and_symlinked_config` | PASS: Profile with missing `config.yaml` or symlinked `config.yaml` raises `QualificationError("lab-profile")` | `tests/test_telegram_monitor_cli_plugin.py:5918` |
| D14 Current-worker selector rejection | `test_d14_normalize_profile_refuses_relative_path_and_worker_identity_substitution` | PASS: Relative path and `HERMES_HOME=implementer` with `HERMES_PROFILE=morfeo` refused | `tests/test_telegram_monitor_cli_plugin.py:5938` |
| D14 Provisioned reference environment access stripping | `test_d14_provisioned_reference_environment_strips_access` | PASS: `_provisioned_reference_environment` roots at profile home and strips all `TELEGRAM_*` and `AETHER_ROUTER_*` variables | `tests/test_telegram_monitor_cli_plugin.py:5960` |
| D14 Negative control on injected lab access | `test_d14_negative_control_reference_probe_rejects_injected_lab_access` | PASS: Supplying `TELEGRAM_HOME_CHANNEL` or `TELEGRAM_BOT_TOKEN` to reference probe raises `QualificationError("lab-reference-access-rejected")` | `tests/test_telegram_monitor_cli_plugin.py:5982` |
| D14 Native loader call order | `test_d14_native_probe_body_executes_load_hermes_dotenv_before_load_gateway_config` | PASS: `load_hermes_dotenv()` syntactically and functionally precedes `load_gateway_config()` | `tests/test_telegram_monitor_cli_plugin.py:6011` |
| D14 Target digest equality | `test_d14_target_digest_equality_and_no_access_values` | PASS: Target digest matches between lab route and provisioned reference (`36007c9a8b0f8394...`), no access values leaked | `tests/test_telegram_monitor_cli_plugin.py:6023` |
| D14 Real runtime reference probe | `test_d14_runtime_reference_destination_probe_matches_gateway_context` | PASS: Reference probe executed against installation runtime resolves `36007c9a8b0f8394...` with `destination_thread_present=False` | `tests/test_telegram_monitor_cli_plugin.py:6043` |
| D14 Preserved prior refusal receipt untouched | `test_d14_preserved_prior_refusal_receipt_and_log_unchanged` | PASS: Preserved receipt SHA-256 `eb2f3ad3...` and log SHA-256 `5a9ca1fe...` unchanged | `tests/test_telegram_monitor_cli_plugin.py:6062` |
