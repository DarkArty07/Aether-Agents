...
- **Unit:** PR8-CRON (`t_2a1d88d1`, rework `t_02092d77`)
...
Four distinct, separately attributed commits on branch `fix/pr8-cron-script-root-origin-cwd`:
...
- **GREEN Evidence on candidate (`4fc5141f8d`):**
  - Command: `HERMES_TEST_FILE_RETRIES=0 uv run pytest -q tests/cron/test_cron_script_root_372.py tests/cron/test_script_claim_heartbeat.py`
  - Result: **23 passed in 3.12s** (9 passed in `test_cron_script_root_372.py`, 14 passed in `test_script_claim_heartbeat.py`).
  - Verified:
    ...
    - Dynamic resolver routes through `scheduler._get_hermes_home()`, ensuring scheduler monkeypatches and profile runtime changes are honored at call time.
...
- **GREEN Evidence on candidate (`4fc5141f8d`):**
  ...
  - Verified:
    ...
    - Gateway origin preserves the complete route in the subscription row (`chat_type`, `thread_id`, `user_id`, `user_id_alt`, `message_id`) without dropping fields or falling back to defaults.
    - Telegram DM origin with thread produces the required topic fallback metadata (`telegram_dm_topic_reply_fallback=True`, `direct_messages_topic_id`, `telegram_reply_to_message_id`).
...
```
**Result:** **222 passed, 1 skipped in 21.84s** (0 retries).

Full directory check:
- `HERMES_TEST_FILE_RETRIES=0 uv run pytest tests/cron/`: **729 passed, 1 skipped, 1 failed (inherited pre-existing `test_every_dispatcher_kanban_var_is_identity_gated` in `test_cron_kanban_env_isolation.py:443`)**.

Static checks on fork:
...
| --- | --- | --- |
| `cron/script_root.py` (new) | #372 | Implements `get_effective_scripts_dir`, `resolve_cron_script_path`, and `validate_cron_script_path`; dynamic resolver honors `scheduler._get_hermes_home()` |
...
| `tools/kanban_tools.py` | #393 | In `_maybe_auto_subscribe`: preserves request-local notification origin from cron context exactly (platform, chat_id, chat_type, thread_id, user_id, user_id_alt, message_id) without env overwrite |
